...
description: Use DWeis WikiGraph knowledge contexts through the managed wg command.
---

# WikiGraph Knowledge

Use this skill whenever the current turn includes a DWeis knowledge context such as `@Knowledge library`, `@知识库`, or a specific imported `.wikg` archive.

## Context scopes

- `wikg://lib` means the whole DWeis local WikiGraph library. Use it for broad questions, cross-archive lookup, discovery, or when the user says only `@知识库` / the knowledge library.
...
- Run WikiGraph only through DWeis's managed `wg` command on PATH.
...
- Do not expose DWeis managed storage paths unless the user explicitly asks for low-level debugging details.
