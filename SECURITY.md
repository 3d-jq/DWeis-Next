...
DWeis is a desktop Agent with access to local files, shell commands, model credentials, and—when a
...
- Risky OpenCode permissions are connected to DWeis's explicit approval UI.
...
an external service that do not cross a DWeis trust boundary should use the relevant project's
normal support channel. Upstream vulnerabilities that affect DWeis's pinned dependency versions
are in scope when the report explains the reachable impact in DWeis.
