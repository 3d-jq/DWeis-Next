...
- Main-process business logs uniformly use the `console.*("[dweis] ...")` prefix — no exceptions
  remain (`electron/protocol.ts` and `electron/preload.ts` also use `[dweis]`).
...
- **Permission `"ask"` must be verified against the UI**: `permission.asked` /
  ...
  sensitive sub-paths. Third-party Python dependencies must use the exact private `.dweis-python`
...
- **oo CLI fast path**: OOMOL keeps the OpenCode fast pass for commands whose first token is `oo` /
  `$DWEIS_OO_BIN` / `${DWEIS_OO_BIN}`. OpenConnector keeps `bash: "ask"` so the main process can
...
- Embedded tool source (`tool-sources.ts`, String.raw) **must not contain backticks or `${}`**
  ...
  knowledge access is intentionally not an OpenCode custom tool: the sidecar receives a DWeis-owned
...
- **Artifacts are only the real files registered by the system**: producers write into each turn's
  ...
  `<project>/.dweis/artifacts/<session>/<turn>/`, and on completion the main process publishes the
  ...
  DWeis's control and may write runtime-state sidecars (session, resume, checkpoint, etc.) into the
...
- **User attachments are separated from model representations**: when an ordinary file is selected
  it is first copied into a DWeis-private 0400 read-only snapshot; preview, parsing, and agent
  ...
  DWeis's synthetic attachment context must be filtered by structured markers — never guessed from
...
- Changes to the vite build config must preserve the invariants: build output defaults to
  oomol.com, is unaffected by `.env.local`, and only an explicit `DWEIS_ENDPOINT` can override it
