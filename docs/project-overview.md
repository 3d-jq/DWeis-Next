# Project Overview: What DWeis Is, Why, and Its Place in the Ecosystem
...
DWeis is an Electron desktop AI Agent chat client. The user states a need in natural language; the Agent
...
- **Target users and the problem solved**: non-developers (knowledge workers in operations, analytics,
  ...
  is tedious and hard to automate. DWeis turns all of that into one natural-language sentence —
...
- **Credential ownership**: with the OOMOL Link runtime, SaaS credentials remain hosted by OOMOL;
  with OpenConnector, provider credentials remain owned by that external runtime. DWeis never stores
...
| **OpenConnector** | Open-source sibling in the same connector ecosystem ([github.com/oomol-lab/open-connector](https://github.com/oomol-lab/open-connector)). Users may select an existing OpenConnector deployment as DWeis's Link runtime by configuring separate API and Console origins plus an optional runtime token. OpenConnector supplies connector actions, not an LLM, and DWeis does not launch, bundle, migrate, or administer it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **WikiGraph**     | The parsing and retrieval runtime for local `.wikg` knowledge bases. `wiki-graph@0.4.0` and `wiki-graph-core@0.4.0` ship as exact-version npm dependencies; the main process uses the SDK directly, and the Agent sees a DWeis-owned `wg` command injected at the front of the OpenCode sidecar `PATH`. That command is built as `dist-electron/dweis-wg.js`, runs through Electron's Node mode, binds DWeis's private `userData/wikigraph-state`, and passes terminal streams/exit code through without wrapping WikiGraph business output. Knowledge-base imports are copied into WikiGraph's managed default library. Currently a Beta feature, off by default — the knowledge-base menu appears and sessions may reference knowledge bases only after the user enables it in Settings; the toggle persists in the local `settings.json`. Sessions persist only knowledge-base IDs and never send the raw archive as a chat attachment.                                                                                               |
| **oo-desktop**    | Sister app + engineering baseline (separate repository, not inside this repo; local path varies per dev machine). DWeis was created as a new standalone repository (not a fork), but build/packaging/CI/IPC service layout/UI style all align with oo-desktop so the two apps' UIs stay coherent and maintenance cost stays low. Mind the known difference: connector auth headers (DWeis uses `Authorization: Bearer <session token>` **without** `x-oomol-user-uuid`; oo-desktop sends the auth.toml account key as a bare header — do not copy it).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **OpenCode**      | The Agent kernel. Spawns the published binary `opencode-ai@1.17.13` as a sidecar; the main process drives it over HTTP+SSE via `@opencode-ai/sdk@1.17.13`. Pure configuration-level customization, zero source modification. At research time (2026-05) no community precedent was found for using OpenCode in non-IDE / general-agent workloads; DWeis was the first known case (positioned as a non-coding agent at inception; local coding capability was opened up later).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

## 3. Original Plan vs Shipped

The original plan document `DWEIS_PROJECT_PLAN.md` (a historical document that existed only on the
...
`dweis://signin` (dev `dweis-local`); IPC over `@oomol/connection`. (The original "three panes, no drag
