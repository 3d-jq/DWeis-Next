...
- The backend API takes Console's existing Skill / team / connection design as reference, but the
  DWeis frontend integrates it into the current three-pane layout and Skills Route structure.
...
- DWeis already has a more complete Skill page and local runtime management; the team layer should
  be added incrementally, not replaced by Console's single-list page.

## 3. Current DWeis foundation

DWeis already has these building blocks:
...
- Runtime Skills are installed to the DWeis runtime skill root via `SkillServiceImpl`, and an
...
For Skills to truly enter the Agent runtime, DWeis ultimately needs a downloadable, verifiable
...
checksum/manifest generation there. DWeis only downloads the single Skill artifact per the
...
- `teamSkillsApiEnabled()` — feature switch read from `VITE_DWEIS_TEAM_SKILLS_API` (legacy alias
  `VITE_DWEIS_ORGANIZATION_SKILLS_API`); `0` / `false` / `off` disable it.
...
cache plus a persisted cache under `dweis.team-skill-cache.v3`, 24 h max age, capped entries;
the legacy `dweis.organization-skill-cache.v2` key is migrated). Switching teams must clear the
...
- Data source tabs:
  ...
  - Public: reuses DWeis's existing public catalog.
...
The complete solution syncs team Skills into DWeis's app-private runtime skill root so OpenCode
...
DWeis restores a valid team id stored for the account. If that selection is missing or no longer
...
- The backend or DWeis can check whether a team Skill has a newer version.
...
- `team-skills-client.ts` exists in DWeis.
