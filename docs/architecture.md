...
- **Main process** `electron/main.ts`: the assembly root. It creates
  ...
  `dweis-resource` artifact protocol is wired in two steps: `registerArtifactResourceScheme()` runs
  at module top level before app ready, and `installArtifactResourceProtocol(artifactResourceLeaseStore)`
  runs in `whenReady`; ChatService then hands the renderer leased `dweis-resource` URLs to stream
  ...
  generation). Deep-links (dev `dweis-local://` / prod `dweis://`) now serve two flows: `handleDeepLink`
  ...
  only allows audio-only requests from the DWeis main window, a trusted renderer URL, and the main
...
- **preload** `electron/preload.ts`: paper-thin. `setupConnectionPreload()` (the @oomol/connection
  RPC bridge) + a contextBridge that exposes a **single** bridge, `window.dweisnext` (name from
  `branding.windowBridge`) — there is no `window.electron` anymore. Beyond `{ appCommit, platform,
version }` (from the vite defines `__APP_COMMIT__` / `__APP_VERSION__`), the `DWeisBridge`
...
survives restart. App-side knowledge-base management uses the `wiki-graph-core` SDK against DWeis's
private `<userData>/wikigraph-state` default library; DWeis does not keep a parallel archive
...
`wikg://lib/arc/<id>` archive URIs. Agent-side WikiGraph access remains through a DWeis-owned `wg`
command placed at the front of the sidecar `PATH`; it binds DWeis's private WikiGraph state
...
Link batch reliability is guaranteed by DWeis's custom `call_action` tool, not left to the model or
...
Beyond the CLI, `search_actions` also fetches `${DWEIS_CONNECTOR_URL}/v1/providers` directly to
...
`DWEIS_TEAM_SCOPE_PATH`, which is how the team can switch without a sidecar restart), and via
...
cb)`. The actual ServiceName string looks like `dweis/chat-service` (prefix from
...
browser settings actions. Page contents have no DWeis preload or Node integration.
...
- The renderer's `oomolFetch(url, init)` in `src/lib/oomol-http.ts` forces `credentials:"include"` —
  ...
  **Never** set an `Authorization` / `Cookie` header in the renderer (`Cookie` is a forbidden header
  ...
  `oomolFetch` dispatches a window `CustomEvent` `dweis:auth-required` (`oomolAuthRequiredEventName`,
...
with `agentPath` as internal input only. OpenCode's expanded synthetic Read text, DWeis's model-only
...
the DWeis attachment manifest, so mid-send, on refresh, and after restart it always shows the file
...
`<project>/.dweis/artifacts/<session>/<turn>/`; with no project, a mismatched project registration,
or Plan mode it stays at `userData/agent/artifacts/<session>/<turn>/`. The project path must pass the
registration check first, and no existing path segment of the project's `.dweis/artifacts` may be a
...
cleans the `.dweis` managed directory, not the visible files. No-project and Plan mode do not
...
appends a dedicated system prompt for that turn and generates `dweis-bug-report.md` based only on the
...
the `dweis:auth-required` window event, `useAuth` listens and calls the AuthService facade's fourth
...
remains retryable, but DWeis does not poll or auto-retry the external flow. OOMOL keeps the existing
...
electron/
  ...
  artifact-resource/ lease-store,protocol(+test)  custom dweis-resource protocol streaming leased artifact files to the renderer
  ...
  update/  common,node,channel,policy(+test)  UpdateService: check/download/install/channel switch, periodic/foreground/wake scheduling + appUpdateStateChanged;
                              generic feed = static.<ep>/release/apps/dweis/<plat>/<arch>; packaged-only; autoDownload=false
...
src/
  ...
  lib/     oomol-http domain  renderer-direct request base (§4): oomolFetch (credentials:include) + the dweis:auth-required 401 broadcast / domain re-export
  ...
  i18n/    in-house lightweight i18n (zh-CN baseline + en, localStorage key dweis.locale)
