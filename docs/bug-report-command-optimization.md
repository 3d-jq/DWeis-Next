...
>
> Scope: DWeis's built-in `/bug-report`, OpenCode 1.17.13, and the command/Skill mechanisms of
...
DWeis's current "bug report" **is not a Skill and never invokes a Skill**. It is an
**application built-in command**: exposed by the DWeis UI, recognized by the main process, and
...
trusted DWeis runtime metadata, a controlled artifact directory, and strict tool boundaries —
...
3. Execution produces no `skill` tool call; DWeis's existing Skill activity UI only recognizes
...
building a **structured DWeis Command**. The "Bug icon + Bug report" chip and the Composer
...
from that definition — but inside DWeis, the command adapter layer stays responsible for
...
6. requires the model to write a single file, `dweis-bug-report.md`, based only on session
...
**DWeis-injected system prompt**, not from any `SKILL.md`.

### 2.3 Why There Is No Skill UI

DWeis Skills have two kinds of visible evidence:
...
- **Trusted metadata**: DWeis version, build commit, platform, model, agent mode, and permission
  mode come from the main process instead of being guessed by the model.
- **Controlled artifact path**: the target file path is assigned by DWeis and enters the existing
  artifact lifecycle.
...
These are all sufficient reasons to keep the "DWeis command orchestration layer".
...
whether DWeis recognized it as a structured command; later diagnostics also lack an explicit
command id/version.
...
The SDK DWeis pins to, 1.17.13, also exposes both a `/command` listing endpoint and a
...
### 4.5 What This Means for DWeis

Mainstream practice supports the current "command triggers a prompt workflow" technical
direction; the flaw is not "it wasn't built as a Skill" but that DWeis still lacks clear product
semantics and execution observability.

At the same time, the trend across Claude Code, VS Code, and Copilot shows users will
increasingly read `/` as a unified capability entry, not a strict type entry. DWeis should
...
Approach: add `dweis-bug-report/SKILL.md`; selecting it adds a Skill chip, then the model calls
...
trustworthily assign DWeis artifact paths or provide main-process runtime metadata; tool
...
Cons: DWeis currently depends on `promptAsync` for streaming, cancellation, permissions, team
...
### Option D: Structured DWeis Command + Single Workflow Definition (Recommended)

Approach: the DWeis UI and IPC use a structured command id; the main process stays responsible
...
show the command id, workflow version, DWeis version, and whether a restricted Build was used —
but not the report body or sensitive context.

### 6.4 After Completion

Keep using the existing single-file artifact card to display `dweis-bug-report.md`. The assistant
...
1. a DWeis built-in, versioned Markdown prompt template;
...
1. Create a non-committed smoke script under `.dweis-dev/` that registers a minimal custom
   command.
2. Verify whether `session.command()`:
   ...
   - can merge with DWeis's per-turn team, project, permission, and artifact system context;
...
5. Based on the Phase 0 result, decide whether the template is carried by the DWeis prompt
...
Execute only if agents outside DWeis genuinely need to reuse the report workflow:

1. Generate or wrap `dweis-bug-report/SKILL.md` from the same workflow spec.
2. The Skill owns the report method, template, and privacy rules — never faking DWeis runtime
   metadata or the artifact path.
3. The DWeis command explicitly calls an internal adapter; show Skill activity only if a Skill
   is actually invoked underneath.
4. Run minimal compatibility tests against Claude Code, OpenCode, and Copilot.

Acceptance: the Skill and the DWeis command never duplicate the spec; every UI accurately
...
- the UI clearly calls it a Command and keeps the same identity through selection, execution,
  and history;
- users can confirm the command was recognized by DWeis, not merely that text was inserted;
...
```text
Slash menu entry
  → structured DWeis Command (deterministic, trusted metadata, safety policy, status UI)
    → versionable report workflow (prompt template; exportable as a Skill when needed)
      → current OpenCode session
        → host-verified dweis-bug-report.md artifact
```
