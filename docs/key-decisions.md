...
- **Decision**: the endpoint became the vite `define` compile-time constant `__OO_ENDPOINT__`;
  ...
  `DWEIS_ENDPOINT` environment variable (**effective in every mode, including build**) >
...
- **Decision**: remove the `@oomol-lab/oo-cli` npm dependency; `OO_CLI_VERSION` in
  ...
  and packaging. Dev resolution order: `DWEIS_OO_BIN` override > `.oo-bin/oo`; `which oo` was
...
- **Decision**:
  - The parameter problem's root cause was the toolset lacking schema-query capability
    ...
    DWeis creates its own `wg` shim under the sidecar private bin directory, prepends that directory
    to `PATH`, and calls WikiGraph's SDK runner with DWeis's private state dir. The shim forwards
    stdin/stdout/stderr/exit code like a terminal command; DWeis does not wrap WikiGraph business
    errors or expose `DWEIS_WIKIGRAPH_*` / `WIKIGRAPH_STATE_DIR` as agent-controlled overrides.
...
- **Decision (the current permission model)**: lift the "three-layer lockdown" (each layer
  ...
  default `"*": "ask"`, but pure oo CLI invocations (`oo`, `oo *`, `$DWEIS_OO_BIN` and their
...
- **Rationale (key constraints)**: OpenCode permission values are `ask | allow | deny`. DWeis's
...
- **Consequences**: the current security posture converged from "any shell / file IO / network
  ...
  Python requirements use the exact turn-private `.dweis-python` interpreter or an exact selected
  ...
  without DWeis pinning a version. Node.js and Python package runners are ordinary local execution
...
- **Background**: DWeis originally derived connector capability, OOMOL model access, account state,
