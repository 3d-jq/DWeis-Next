...
DWeis Default Access decides dependency permissions from execution scope, package source, and
...
- The target is the exact interpreter in the active turn's private `.dweis-python` environment or
...
- Package popularity is irrelevant, and DWeis does not add or pin a version.
...
- Ordinary versions and package-manager flags are accepted. DWeis does not add or pin a version.
...
```bash
python3 -m venv <task-directory>/.dweis-python &&
  <task-directory>/.dweis-python/bin/python -m pip install python-docx 2>&1
```
...
The same structural rule applies to non-dependency risk checks. DWeis identifies the executable and
