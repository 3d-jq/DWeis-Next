...
- `pnpm install`'s postinstall chains the binary/skill download scripts and finally builds the
  ...
  - `scripts/download-electron.ts` → downloads the dev-only Electron copy into `.electron-dist/`
    and rewrites the macOS Info.plist to `com.oomol.dweis-local` / the `dweis-local` scheme (for
  ...
  - `scripts/download-ripgrep.ts` → downloads ripgrep into `.oo-bin/` (version pinned by
    ...
    `OO_SKIP_BINARY_DOWNLOAD=1` or `DWEIS_SKIP_RIPGREP_DOWNLOAD=1` skips it.
...
- `DWEIS_ENDPOINT`: the endpoint apex domain, default `oomol.com`; switch to `oomol.dev` to target
  ...
  `DWEIS_ENDPOINT=oomol.dev corepack pnpm run build`. Known trap: the `oomol.dev` LLM gateway once returned
...
- `DWEIS_OO_BIN` (optional; a process environment variable, not read from .env files): overrides
  the oo binary path, `DWEIS_OO_BIN=/abs/path/to/oo corepack pnpm run dev`. When set, it only skips the
...
- `DWEIS_PACKAGE_ASSETS_BASE_URL` (optional): overrides the package-assets base URL, injected as
...
corepack pnpm run dev    # predev runs scripts/check-oo.ts first (three checks, see below)
               ...
               # Electron userData is the repo-local ./dweis directory
...
- `corepack pnpm run dev:worktree` keeps the same `./dweis` userData rule for each worktree while
  ...
  or empty, it copies the canonical main repo's `./dweis` once as an initialization source; it never
  shares or overwrites an existing worktree userData directory. `DWEIS_DEV_AUTH_SOURCE_DIR=/abs/path`
  can override that initialization source.
- Code changes must go through a temporary branch + PR: first align local `main` with
  `origin/main`, then cut a short branch from `main` (at most three words, hyphen-separated,
  no slashes or type prefixes — e.g. `settings-apple-polish`, `agent-restart-cooldown`,
  `refactor-token-store`). Once the change is done and passes the quality gate, push the temporary
  branch and open a PR to `3d-jq/DWeis-Next:main`; land it by merging the PR. Never commit or push
...
- **UI real-machine verification bypasses** (dev-only env vars, harmless in production):
  ...
  `VITE_DWEIS_LOCALE` (forces the locale, `src/i18n/i18n.ts`); pair with macOS `screencapture` for
...
- `pnpm run lint` = `oxlint .` (`.oxlintrc.json`: correctness=error; `react/only-export-components`
  ...
  `src/components/ai-elements/**`; ignorePatterns includes `.dweis-dev`).
...
- Auto-update (`electron/update/`: common.ts contract + node.ts implementation + channel.ts /
  ...
  `https://static.<ep>/release/apps/dweis/<platform>/<arch>`; packaged builds only. First check
  ...
  the DWeis state machine controls the download explicitly, and `autoInstallOnAppQuit=true` is
...
- **release.yml** (workflow_dispatch, inputs channel stable/beta + expected_version +
  ...
  upload to Aliyun OSS `oomol-static-cn-prod/release/apps/dweis`, OIDC) + `release-win`
...
| `.electron-dist/`               | dev-only Electron copy (dweis-local scheme)                                            | postinstall `scripts/download-electron.ts`                           |
...
| `.dweis-dev/`                   | manual smoke / experiment scripts, outside every toolchain                             | handwritten                                                          |
| `dweis/`                        | dev Electron userData for this checkout                                                | `pnpm run dev` / `pnpm run dev:worktree`                             |
...
- Upgrading OpenCode: bump `opencode-ai` / `@opencode-ai/sdk` / `@opencode-ai/plugin` together at
  the **same version**, and run the `.dweis-dev/` smoke scripts first (upstream makes no API
  stability promise).
