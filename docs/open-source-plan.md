# DWeis Open-Source and Login-Free Mode Implementation Plan
...
> Goal: turn DWeis into an open-source desktop app that is login-free by default and supports BYOK
...
DWeis today is OOMOL's Electron desktop AI Agent client. The chat UI, OpenCode sidecar, local
...
- the closed loop between OpenCode permission ask and DWeis's permission UI;
...
- DWeis is not just a chat-bubble UI — it is a complete desktop Agent client;
...
Result: DWeis's chat and local Agent core are usable without signing in.
