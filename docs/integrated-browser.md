...
This is a behavioral risk policy, not a system-level guarantee. DWeis must not claim that every
...
execution semantics; DWeis does not reimplement them.

## Explicit Invocation

Add Browser as a DWeis-owned built-in Skill that users can select with `$browser`. The Skill
...
- Keep the source in the tracked `resources/dweis-skills/browser/SKILL.md`; the existing build step
...
The panel chrome and the native view's blank, loading, and transparent surfaces follow DWeis's
effective light or dark theme. Electron also exposes that theme to compatible pages through
`prefers-color-scheme`. DWeis does not inject replacement colors into arbitrary websites or
...
import Electron. The runtime-supplied OpenCode session ID is mapped to the DWeis chat session on the
server; the model cannot select another task's page.

Use a DWeis-owned persistent Electron partition, separate from `defaultSession`, for browser
...
- Ordinary downloads are saved to the operating system's Downloads folder. DWeis reports
...
- Browser contents have no DWeis preload and no Node.js integration.
...
The V1 browser uses its own persistent profile. Users sign in once inside DWeis and reuse that
state.

DWeis does not read or copy Chrome's profile database. Chrome cookies are operating-system
...
is a Chrome extension that lets DWeis operate the user's current Chrome tab without moving
...
- Browser content cannot access DWeis internals, local files, credentials, or denied device
  permissions.
