...
>
> Scope: let DWeis use a user-configured OpenConnector runtime as its Link
...
DWeis currently couples three concerns that need independent ownership:
...
- The deterministic same-`OO_DATA_DIR` A -> B -> A probe returned the A schema,
  ...
  DWeis keeps the existing isolated Agent `OO_DATA_DIR` and does not add a
...
- DWeis's Link tools currently require an OOMOL team identity and append
  --organization to apps and run calls.
- DWeis's provider metadata request reads OO_API_KEY, while OpenConnector uses
  OO_CONNECTOR_TOKEN and may also run without authentication.
- DWeis recognizes the hosted connector authorization error codes but not
  OpenConnector's connection_not_found, oauth_token_expired,
  oauth_refresh_unavailable, and authorization_failed codes.
- DWeis currently opens its internal OOMOL connection drawer for every
...
process isolation between bash and OO_CONNECTOR_TOKEN. Instead, DWeis must
...
DWeis must not store the OpenConnector admin token, OAuth client secrets, or
...
- introduce an OpenConnector admin-token boundary into DWeis.
...
- Run DWeis once and verify the current OOMOL, local custom-model, and
...
- The production implementation keeps one isolated Agent oo store and adds
  backend plus normalized endpoint identity to DWeis's in-memory inventory,
...
- DWeis then launched against a temporary Electron userData directory with the
...
OO_CONNECTOR_TOKEN
DWEIS_CONNECTOR_URL=<baseUrl>
DWEIS_CONSOLE_URL=<consoleUrl>
DWEIS_LINK_RUNTIME=openconnector
```
...
- query_knowledge remains available in an OpenConnector runtime and reads
  sessionKnowledgeBaseIds from DWEIS_TEAM_SCOPE_PATH.
...
- query_knowledge remains independent of Link identity and continues reading
  sessionKnowledgeBaseIds from DWEIS_TEAM_SCOPE_PATH in every Agent runtime.
...
- call oo connector login to persist DWeis configuration;
...
- ownership between DWeis and the OpenConnector Console.
...
parsing inside DWeis's embedded tool strings.
