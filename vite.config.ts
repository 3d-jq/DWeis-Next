...
// Electron（带 dev 专用 Bundle ID / URL scheme = dweis-local）。仅当该副本存在
...
function resolveAppCommit(): string {
  const fromEnv = process.env.DWEIS_BUILD_COMMIT ?? process.env.GITHUB_SHA
...
//   1) 显式 DWEIS_ENDPOINT 环境变量（shell / CI），两种模式都生效；
...
function resolveOoEndpoint(command: string, mode: string): string {
  const explicit = process.env.DWEIS_ENDPOINT?.trim()
  if (explicit) {
    return explicit
  }
  if (command !== "build") {
    const fromFile = loadEnv(mode, dirname, "").DWEIS_ENDPOINT?.trim()
    if (fromFile) {
      return fromFile
    }
  }
...
function shouldAutoStartElectron(command: string, mode: string): boolean {
  ...
  const explicit = process.env.DWEIS_ELECTRON_AUTO_START?.trim().toLowerCase()
  if (explicit) {
...
function resolveDevServerPort(): number {
  const explicit = process.env.DWEIS_DEV_SERVER_PORT?.trim()
  if (!explicit) {
    return 5273
  }
  ...
  throw new Error(`DWEIS_DEV_SERVER_PORT must be an integer from 1024 to 65535, got "${explicit}"`)
}

function skipElectronStartup(): void {
  console.log("[dweis] Electron auto-start disabled for this Vite dev session.")
}
...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      {
        name: "dweis-inline-boot-theme-key",
        transformIndexHtml(html: string) {
          return html
            .replaceAll("%APP_NAME%", branding.appName)
            .replaceAll("%DWEIS_THEME_KEY%", storageKey("theme"))
            .replaceAll("%DWEIS_THEME_PALETTE_KEY%", storageKey("theme-palette"))
        },
      },
      tailwindcss(),
      react(),
            ...
            build: {
              rollupOptions: {
                input: {
                  ...
                  "dweis-wg": path.join(dirname, "electron/knowledge/wg.ts"),
                },
                ...
                external: [
                  ...
                  /^wiki-graph-core(\/|$)/,
                  "electron-updater",
                  // 内置模块：vite 打包会把 node:sqlite 的导出弄丢（DatabaseSync 变 undefined），
                  // 必须保持外部化由 Node 运行时解析（Electron 42 = Node 22.19，内置可用）。
                  "node:sqlite",
                ],
              },
            },
