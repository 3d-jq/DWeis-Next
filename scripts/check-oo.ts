...
// 设了 DWEIS_OO_BIN 覆盖时跳过检查（信任开发者指定的路径）。
...
if (!process.env.DWEIS_OO_BIN) {
  ...
  if (!existsSync(ooBin)) {
    console.error(
      `[dweis] oo 二进制缺失：${ooBin}\n` +
        "  运行 `pnpm run postinstall` 重新下载，或设 DWEIS_OO_BIN 指向已有 oo（见 .env.example）。",
    )
    process.exit(1)
  }
...
if (!bundledSkillsReady) {
  try {
    ...
    console.log(`[dweis] bundled skills ready at ${bundledSkillsDir}`)
  } catch (error) {
    console.warn("[dweis] failed to export bundled skills (non-fatal):", error)
  }
}
