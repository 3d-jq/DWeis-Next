...
const cleanProfileMarkerName = ".dweis-clean-profile"
...
if (isMainModule()) {
  void main(process.argv.slice(2)).catch((error: unknown) => {
    console.error(`[dweis] ${error instanceof Error ? error.message : String(error)}`)
    process.exitCode = 1
  })
}
...
async function main(args: string[]): Promise<void> {
  ...
  switch (command) {
    ...
    case "restore":
      throw new Error(
        `auth:${command} is deprecated. Dev Electron userData now lives at ./dweis, and dev:worktree initializes from the canonical repo ./dweis when the target is empty. This command no longer reads or writes ~/dweis-dev.`,
      )
    default:
...
export async function resolveDevUserDataDir(config?: BootstrapConfig): Promise<string> {
  ...
  const configuredUserData = loadedConfig?.userDataDir ?? loadedConfig?.env?.["DWEIS_USER_DATA_DIR"]
  return path.resolve(repoRoot, configuredUserData?.trim() || "dweis")
}
...
async function readBootstrapConfig(): Promise<BootstrapConfig> {
  const bootstrapJsonPath = path.join(repoRoot, ".dweis-dev", "bootstrap.json")
...
async function cleanDevUserData(userDataDir: string): Promise<void> {
  ...
  console.log(`[dweis] reset dev userData to a clean signed-out state: ${userDataDir}`)
}

async function printStatus(userDataDir: string): Promise<void> {
  console.log("[dweis] auth snapshot commands are deprecated; dev userData now lives in ./dweis")
...
function printAuthState(label: string, dir: string, state: AuthState): void {
  ...
  console.log(`[dweis] ${label}: ${status}`)
  console.log(`[dweis]   dir: ${dir}`)
  console.log(`[dweis]   profile: ${state.hasProfile ? "present" : "missing"}`)
  console.log(`[dweis]   oomol-token cookie marker: ${state.hasOomolCookie ? "present" : "missing"}`)
  console.log(`[dweis]   oomol-token expires: ${formatExpiry(state.oomolCookieExpiresAtMs)}`)
}
...
export async function createAuthFixture(root: string): Promise<string> {
  const dir = await mkdir(path.join(root, "dweis-auth-fixture"), { recursive: true })
  const userDataDir = dir ?? path.join(root, "dweis-auth-fixture")
  await writeFile(
...
export function tempAuthRoot(): string {
  return path.join(os.tmpdir(), `dweis-auth-${process.pid}`)
}
