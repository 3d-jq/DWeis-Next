...
// 再补入 DWeis 自带的只读工作流 skills（resources/dweis-skills/）。
// 供 dev 与打包共用。运行时由 electron/agent/workspace.ts 拷进 OpenCode workspace 的 .opencode/skill/，
// 使 DWeis 自己的 agent 直接读到这些 skill——不再像旧 oo-cli 那样把 skill 释放到其他 AI agent 家目录。
//
...
export const dweisSkillsDir = path.join(repoRoot, "resources", "dweis-skills")

// oo-create-skill 已剔除：其工作流依赖已移除的 oo CLI（oo skills init/adopt/validate），由自有 skill-creator 替代。
const ooBundledSkillIds = ["oo", "oo-find-skills", "oo-publish-skill"] as const
// MiniMax-AI/skills 办公 skill（MIT）：PPT/DOCX/XLSX/PDF 生成与编辑，来源 https://github.com/MiniMax-AI/skills。
export const dweisBundledSkillIds = [
  "browser",
  "skill-creator",
  "wikigraph-knowledge",
  "pptx-generator",
...
// 需内置到 DWeis agent workspace 的 skill；用于导出后的完整性校验（数量/缺失）。
export const bundledSkillIds = [...ooBundledSkillIds, ...dweisBundledSkillIds] as const
...
export async function exportBundledSkills(outDir: string = bundledSkillsDir): Promise<string> {
  ...
  const storeDir = path.join(os.tmpdir(), "dweis-oo-skill-export-store")
  ...
  // oo skills install 是全量导出（无法按 id 排除）：已废弃的 oo-create-skill（工作流依赖已移除的
  // oo CLI，由自有 skill-creator 替代）在此主动剔除，避免每次打包/安装后残留。
  await rm(path.join(outDir, "oo-create-skill"), { force: true, recursive: true })
  await Promise.all(
    dweisBundledSkillIds.map((skillId) =>
      cp(path.join(dweisSkillsDir, skillId), path.join(outDir, skillId), { recursive: true }),
    ),
  )
  return outDir
}
