...
const requiredEnvKeys = ["DWEIS_DEV_SERVER_PORT", "DWEIS_SKIP_PROTOCOL_REGISTRATION", "DWEIS_USER_DATA_DIR"]
...
async function main(): Promise<void> {
  ...
  console.warn("[dweis] configured dev server port is already in use; selecting another worktree port")
...
function parseBootstrapConfig(value: unknown): BootstrapConfig {
  ...
  if (Number(value.env["DWEIS_DEV_SERVER_PORT"]) !== value.devServerPort) {
    throw new Error("env.DWEIS_DEV_SERVER_PORT must match devServerPort")
  }
  return {
...
export async function initializeWorktreeUserData(
  ...
  if (!(await isDirectoryMissingOrEmpty(target))) {
    console.log(`[dweis] keeping existing worktree userData: ${target}`)
    return "kept"
  }

  const source = await resolveCanonicalUserDataDir(repoRoot)
  if (source === undefined) {
    console.log("[dweis] canonical dev userData not found; starting with a clean worktree profile")
    return "empty-source"
  }
  if (await isSamePath(source, target)) {
    console.log(`[dweis] current checkout is the canonical dev userData source: ${target}`)
    return "self"
  }
  if (await isDirectoryMissingOrEmpty(source)) {
    console.log(`[dweis] canonical dev userData is empty: ${source}; starting with a clean worktree profile`)
    return "empty-source"
  }

  await copyDirectoryOnce(source, target)
  console.log(`[dweis] initialized worktree userData from canonical source: ${source} -> ${target}`)
  return "copied"
}

export async function resolveCanonicalUserDataDir(currentRepoRoot: string): Promise<string | undefined> {
  const configured = process.env["DWEIS_DEV_AUTH_SOURCE_DIR"]?.trim()
  if (configured) {
  ...
  return canonicalRoot === undefined ? undefined : path.join(canonicalRoot, "dweis")
}
