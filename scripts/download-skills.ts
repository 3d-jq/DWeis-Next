...
//
// 全程 best-effort：任何失败只告警、不阻断 pnpm install（对齐 download-oo.ts）。
...
async function main(): Promise<void> {
  try {
    ...
    console.log(`[dweis] bundled skills ready at ${dest}`)
  } catch (error) {
    console.warn("[dweis] download-skills postinstall failed (non-fatal):", error)
  }
}
