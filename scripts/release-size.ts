...
//                macOS 是 `mac-<arch>/DWeis Next.app`；Windows 是 `win-unpacked`
...
export async function collectMacMetadata({
  ...
  const dmgPath = path.join(releaseDir, `DWeis Next-${version}.dmg`)
  const zipPath = path.join(releaseDir, `DWeis Next-${version}.zip`)
  const appPath = path.join(releaseDir, macUnpackedDirName(arch), "DWeis Next.app")
...
export async function collectWinMetadata({
  ...
  const exePath = path.join(releaseDir, `DWeis Next-${version}-Setup.exe`)
