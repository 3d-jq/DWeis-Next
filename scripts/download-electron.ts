...
async function main(): Promise<void> {
  try {
    ...
    if (await isElectronDistUpToDate()) {
      console.log(`[dweis] .electron-dist already at v${electronVersion}, skip download.`)
      await writeElectronPackagePath()
      if (process.platform === "darwin") {
        await modifyPlist()
      }
    ...
    console.warn("[dweis] download-electron postinstall failed (non-fatal):", error)
  }
}
...
async function modifyPlist(): Promise<void> {
  ...
  try {
    ...
    console.warn("[dweis] mac URL scheme registration failed (non-fatal):", error)
  }
}
