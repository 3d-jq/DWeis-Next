...
const OSS_BASE = "https://static.oomol.com/release/apps/dweis"
...
describe("sumFileBytesRecursive", () => {
  let dir: string
  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), "dweis-release-size-"))
  })
  afterEach(async () => {
...
describe("collectMacMetadata", () => {
  ...
  beforeEach(async () => {
    releaseDir = await mkdtemp(path.join(tmpdir(), "dweis-release-size-mac-"))
  })
  afterEach(async () => {
  ...
  it("produces a two-artifact entry with the same expanded size for DMG and ZIP", async () => {
    const version = "9.8.7"
    await writeFile(path.join(releaseDir, `DWeis Next-${version}.dmg`), Buffer.alloc(1000))
    await writeFile(path.join(releaseDir, `DWeis Next-${version}.zip`), Buffer.alloc(800))
    const appDir = path.join(releaseDir, "mac-arm64", "DWeis Next.app", "Contents", "MacOS")
    await mkdir(appDir, { recursive: true })
    await writeFile(path.join(appDir, "DWeis Next"), Buffer.alloc(2048))
    ...
    assert.deepEqual(metadata.artifacts[0], {
      artifact: "DMG",
      fileName: `DWeis Next-${version}.dmg`,
      downloadBytes: 1000,
      expandedBytes: 2048,
    ...
    assert.deepEqual(metadata.artifacts[1], {
      artifact: "ZIP",
      fileName: `DWeis Next-${version}.zip`,
      downloadBytes: 800,
      expandedBytes: 2048,
      expandedLabel: "app bundle",
    })
  ...
  it("fails fast when the unpacked .app bundle is missing", async () => {
    const version = "9.8.7"
    await writeFile(path.join(releaseDir, `DWeis Next-${version}.dmg`), Buffer.alloc(1))
    await writeFile(path.join(releaseDir, `DWeis Next-${version}.zip`), Buffer.alloc(1))
...
describe("collectWinMetadata", () => {
  let releaseDir: string
  beforeEach(async () => {
    releaseDir = await mkdtemp(path.join(tmpdir(), "dweis-release-size-win-"))
  })
  afterEach(async () => {
    await rm(releaseDir, { recursive: true, force: true })
  })
  ...
  it("produces Setup EXE entries with the same expanded size", async () => {
    const version = "9.8.7"
    await writeFile(path.join(releaseDir, `DWeis Next-${version}-Setup.exe`), Buffer.alloc(500))
    ...
    assert.deepEqual(metadata.artifacts[0], {
      artifact: "Setup EXE",
      fileName: `DWeis Next-${version}-Setup.exe`,
      downloadBytes: 500,
      expandedBytes: 4096,
  ...
  it("fails fast when win-unpacked is missing", async () => {
    const version = "9.8.7"
    await writeFile(path.join(releaseDir, `DWeis Next-${version}-Setup.exe`), Buffer.alloc(1))
...
describe("renderDownloadsTable", () => {
  ...
  const winMetadata: ReleaseSizeMetadata = {
    ...
    artifacts: [
      {
        artifact: "Setup EXE",
        fileName: "DWeis Next-1.2.3-Setup.exe",
  ...
  it("emits Markdown table header, formatted sizes, and OSS download links", () => {
    ...
    assert.ok(table.includes(`${OSS_BASE}/darwin/arm64/DWeis Next-1.2.3.dmg`))
    assert.ok(table.includes(`${OSS_BASE}/win32/x64/DWeis Next-1.2.3-Setup.exe`))
