...
function parseSearch(output: string): { kind: "grep" | "list"; hits: SearchHit[] } {
  ...
  // 通用：尝试每行 grep 格式
  const hits = lines.map((line) => parseGrepLine(line)).filter(Boolean) as SearchHit[]
...
export function SearchCard({ output, t }: SearchCardProps) {
  ...
  if (parsed.kind === "list") {
    return (
      <section className="space-y-0.5 rounded-xl border border-[var(--oo-divider)] bg-muted/30 p-2.5 text-xs">
        ...
        {groups.map(([filePath]) => (
          <div key={filePath} className="rounded px-1 py-0.5 font-mono text-muted-foreground hover:bg-muted/40">
            <span className="text-muted-foreground/50 select-none">·</span> {filePath}
          </div>
        ))}
      </section>
    )
      ...
      {groups.map(([filePath, hits]) => (
        <div key={filePath} className="space-y-0.5">
          ...
          {hits.map((hit, index) => (
            ...
            >
              {hit.line !== null ? (
                <span className="w-10 text-right text-muted-foreground/50 tabular-nums select-none">{hit.line}</span>
              ) : (
                <span className="w-10" />
              )}
