...
import { buildContextUsageBreakdown, buildContextUsageInfo, latestContextTokenUsage } from "./context-usage.ts"
...
interface ChatComposerProps {
  ...
  onUndo?: () => void
  onRedo?: () => void
  /** 用户以 ! 前缀输入命令时直接执行（对齐 opencode shell 直执行）。 */
...
export function ChatComposer({
  ...
  const skillItems = React.useMemo(
    ...
    [appSettings.loading, appSettings.settings.browserEnabled, skillInventory.data?.groups, t],
  )
  ...
  const contextItems = React.useMemo(
    () => buildContextPaletteItems({ artifactItems, knowledgeItems: knowledgePaletteItems, platform, t }),
    [artifactItems, knowledgePaletteItems, platform, t],
  )
  ...
  const handleSubmit = async (message: PromptInputMessage): Promise<void> => {
    ...
    // 输入以 ! 开头 = shell 直执行（对齐 opencode：用户直接跑命令写入会话，不发给 agent）。
    if (text.startsWith("!") && text.length > 1 && attachments.length === 0 && command === null && !composerDisabled) {
      onRunShellCommand?.(text.slice(1).trim())
      dispatchComposer({ type: "reset-after-submit" })
      return
    }
    if (
...
/** 拉取 agent workspace 的自定义斜杠命令（.opencode/command/*.md）。 */
function useCustomCommands(): CustomCommand[] {
  const chatService = useChatService()
  const [commands, setCommands] = React.useState<CustomCommand[]>([])
  React.useEffect(() => {
    let cancelled = false
    void chatService
      .invoke("getCustomCommands")
      .then((result) => {
        if (!cancelled) {
          setCommands(result)
        }
