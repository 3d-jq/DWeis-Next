import type { DWeisReasoningVariant } from "../../../electron/agent/reasoning.ts"
import type {
  CustomModelApiPlan,
  CustomModelProvider,
  CustomModelSummary,
...
function providerDefaultProtocol(provider: CustomModelProvider | undefined, modelName: string): ModelProtocol {
  const option = provider?.modelOptions?.find((model) => model.id === modelName.trim())
...
function providerDefaultReasoningVariants(
  provider: CustomModelProvider | undefined,
  modelName: string,
): DWeisReasoningVariant[] {
  const option = provider?.modelOptions?.find((model) => model.id === modelName.trim())
  const variants = option?.reasoningVariants ?? provider?.reasoningVariants
  // 无预设（如"自定义"供应商）时默认全档：避免漏勾 max 导致推理档位超范围被丢弃、
  // 模型回默认思考模式把思考内联进正文（思考裸露）。
  return variants ? [...variants] : [...DWEIS_REASONING_VARIANT_LEVELS]
}
...
function providerDefaultName(provider: CustomModelProvider | undefined): string {
  ...
  return provider.id === "custom" ? "" : provider.displayName
}
...
function AddCustomModelForm({
  ...
  const t = useT()
  // ── 表单状态：全部从 props 初始化（组件随 key 重挂载，此处即"每次打开"） ──
  // 供应商默认：编辑用其 providerId；添加用预设（无预设 → 自定义，绝不再落到 providers[0]=DeepSeek）。
  const initialProviderId = model?.providerId ?? presetProviderId ?? "custom"
  ...
  const [supportsImages, setSupportsImages] = React.useState(
    model?.supportsImages ?? providerDefaultSupportsImages(initialProvider, model?.modelName ?? ""),
  )
  const [supportsToolCalls, setSupportsToolCalls] = React.useState(
    model?.supportsToolCalls ?? providerDefaultSupportsToolCalls(initialProvider, model?.modelName ?? ""),
  )
  const [contextWindow, setContextWindow] = React.useState(
    String(model?.contextWindow ?? providerDefaultContextWindow(initialProvider, model?.modelName ?? "")),
  )
  const [inputTokenLimit, setInputTokenLimit] = React.useState(
    String(model?.inputTokenLimit ?? providerDefaultInputTokenLimit(initialProvider, model?.modelName ?? "")),
  )
  ...
  /** 切换供应商：全部字段重置为该供应商默认（不保留旧值，杜绝 DeepSeek 残留进自定义名字）。 */
  const handleProviderChange = (nextId: string): void => {
    ...
    setProviderName(providerDefaultName(next))
    ...
    setProtocol(providerDefaultProtocol(next, nextModelName))
  }
  ...
  const save = (): void => {
    ...
    void onSave({
      ...(model ? { id: model.id } : {}),
      providerId,
      providerName: providerName.trim() || providerDisplayName(provider, t),
      // baseUrl 原样透传：用户按自己 URL 选 API 协议格式，应用不改写。
      baseUrl,
      apiKey,
      modelName,
      supportsImages,
  ...
  return (
    ...
    >
      <div className="grid gap-4">
        ...
        {/* 自定义供应商名只有「自定义」类型可编辑；预设供应商（DeepSeek/MiniMax 等）名称固定 */}
        {provider?.id === "custom" ? (
          <div className="grid gap-1.5">
            <Label>{t("chat.modelProviderName")}</Label>
            <Input
              value={providerName}
              onChange={(event) => setProviderName(event.target.value)}
              placeholder={providerDisplayName(provider, t)}
        ...
        <div className="grid gap-1.5">
          <Label>{t("chat.modelProtocol")}</Label>
          <Select value={protocol} onValueChange={(next) => setProtocol(next as ModelProtocol)}>
            <SelectTrigger className={modelDialogControlClass}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent position="popper" className="w-[var(--radix-select-trigger-width)]">
              <SelectItem value="openai">{t("chat.modelProtocolOpenAI")}</SelectItem>
              <SelectItem value="anthropic">{t("chat.modelProtocolAnthropic")}</SelectItem>
              <SelectItem value="responses">{t("chat.modelProtocolResponses")}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="grid gap-2 rounded-md border border-border/70 px-3 py-2.5">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {DWEIS_REASONING_VARIANT_LEVELS.map((variant) => (
