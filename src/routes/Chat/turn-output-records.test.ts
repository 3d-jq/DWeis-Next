...
describe("turn output record grouping", () => {
  ...
  it("keeps the latest record for a multi-assistant turn", () => {
    // u1/u2 空 parts 会被 groupChatTurns 当幽灵跳过（自动压缩触发消息语义），补文本恢复两回合结构。
    const turns = groupChatTurns([
      { ...message("u1", "user"), parts: [{ kind: "text", partId: "u1-t", text: "first" }] },
      message("a1", "assistant"),
      message("a2", "assistant"),
      { ...message("u2", "user"), parts: [{ kind: "text", partId: "u2-t", text: "second" }] },
