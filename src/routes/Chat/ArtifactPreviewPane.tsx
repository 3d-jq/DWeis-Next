...
const ArtifactPptxPreview = React.lazy(() => import("./ArtifactPptxPreview.tsx"))
...
export function ArtifactConsumablePreview({
  ...
  if (preview?.kind === "document" && preview.documentFormat === "pptx" && resourceSource) {
    return (
      <ErrorBoundary key={`${item.path}:pptx`} fallback={lazyPreviewFallback}>
        <React.Suspense
          fallback={
            <div className="oo-text-body flex min-h-full items-center justify-center px-4 py-8 text-muted-foreground">
              {t("artifacts.previewLoading")}
        ...
        >
          <ArtifactPptxPreview source={resourceSource} name={item.name} onResourceError={onResourceError} />
        </React.Suspense>
      </ErrorBoundary>
    )
  }
