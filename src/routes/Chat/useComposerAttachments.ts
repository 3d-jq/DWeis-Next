...
export function useComposerAttachments({
  ...
  const addFiles = React.useCallback(
    async (files: FileList | File[]) => {
      ...
      for (const file of Array.from(files)) {
        const selectedPathForFile = globalThis.dweisnext?.selectedAttachmentPathForFile
        const selected = selectedPathForFile ? await selectedPathForFile(file) : null
        const saver = globalThis.dweisnext?.saveClipboardAttachment
        let optimized: {
          name: string
          mime: string
          size: number
  ...
  const selectAttachments = React.useCallback(
    async (kind: AttachmentPickerKind) => {
      clearInputError()
      const picker = globalThis.dweisnext?.selectAttachmentPaths
      if (!picker) {
        if (kind === "file") {
