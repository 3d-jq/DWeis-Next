...
export interface QuestionField {
  ...
  /** 多选（来自 ChatQuestionInfo.multiple）：选中多个选项并提交多个答案。 */
  multiple?: boolean
}
...
/** plan_exit 等系统确认问题：header 为 "Build Agent"，直接渲染选项（Yes/No），不做字段推断。 */
const DIRECT_OPTION_QUESTION_HEADERS = new Set(["Build Agent"])
...
function inferFieldKind(label: string, prompt: string): QuestionFieldKind {
  ...
  // 注意不要用 \bto\b 判断邮箱：plan_exit 的 "switch to build mode" 等普通问题会被误伤。
  if (/收件人|邮箱|email|recipient|mail address/.test(text)) {
    return "email"
  }
...
function fieldsForQuestion(requestId: string, question: ChatQuestionInfo, questionIndex: number): QuestionField[] {
  // 系统确认问题（plan_exit：header="Build Agent"）直接渲染原始选项，跳过字段推断，
  // 避免 "switch to build mode" 里的 to 被误判成邮箱字段而丢选项。
  if (DIRECT_OPTION_QUESTION_HEADERS.has(question.header)) {
    const label = cleanQuestionText(question.question) || question.header || "选择"
    const field: QuestionField = {
      id: `${requestId}:${questionIndex}:confirm`,
      questionIndex,
      label,
      kind: "text",
      value: "",
      multiple: question.multiple,
      options: usefulOptions("text", question),
    }
    return [field]
  }
  const prompts = extractNumberedPrompts(question.question)
  const fallback = createField(requestId, questionIndex, question.question, [], "", question.header, question.multiple)
  ...
  if (knownBody && !fields.some((field) => field.kind === "textarea")) {
    fields.push(createField(requestId, questionIndex, "正文", [], knownBody, undefined, question.multiple))
  }
...
export function fieldDraftValues(field: QuestionField, draft: QuestionFieldDraft): string[] {
  if (field.multiple && draft.selected.length > 1) {
    return draft.selected
      .map((label) => {
        const option = field.options.find((item) => item.label === label)
        if (!option) {
          return label
        }
        if (option.manual) {
  ...
  const value = fieldDraftValue(field, draft)
  return value ? [value] : []
}
...
export function answersFromFieldDrafts(
  ...
  return request.questions.map((_, questionIndex) => {
    ...
    if (questionFields.length === 1) {
      ...
      return fieldDraftValues(questionFields[0], drafts[fieldIndex] ?? { value: "", selected: [] })
    }
