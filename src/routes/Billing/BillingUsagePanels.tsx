...
export function UsageDetailsDisclosure({
  ...
  const t = useT()
  const canExport = subjectSummaries.length > 0
  return (
            ...
            <BillingPanel
              ...
              action={
                <Button
                  ...
                  onClick={() =>
                    exportUsageCsv(subjectSummaries, period, dataAsOf, {
...
function SubjectBreakdown({
  ...
  const t = useT()
  const [category, setCategory] = React.useState<UsageCategory | "all">("all")
...
function exportUsageCsv(
  ...
  link.href = url
  link.download = `dweis-usage-${period}d-${new Date().toISOString().slice(0, 10)}.csv`
  link.click()
