...
import { DownloadIcon, InfoIcon, RefreshCwIcon, RotateCcwIcon } from "lucide-react"
...
export function AboutSettings({ update }: { update: UseAppUpdate }) {
  const { t } = useI18n()
  ...
  const version = update.state?.currentVersion ?? globalThis.dweisnext?.version ?? "—"
  const platform = globalThis.dweisnext?.platform ?? "browser"
...
export function UpdateChannelSettings({ update }: { update: UseAppUpdate }) {
  const { t } = useI18n()
  return (
...
export function UpdateAction({ update }: { update: UseAppUpdate }) {
  const { t } = useI18n()
