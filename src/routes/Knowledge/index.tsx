...
function knowledgeCoverageTone(percent: number): string {
  ...
  if (percent < 40) return "text-warning/90"
  if (percent < 60) return "text-warning"
  if (percent < 80) return "text-info/90"
  if (percent < 100) return "text-info"
  return "text-success"
}
...
export function KnowledgeRoute({
  ...
  const handleDrop = async (event: React.DragEvent<HTMLDivElement>): Promise<void> => {
    ...
    for (const file of files) {
      const selectedPath = await window.dweisnext.selectedAttachmentPathForFile(file)
