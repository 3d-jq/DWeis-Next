...
function logSettledFailure(label: string, result: PromiseSettledResult<unknown>): void {
  if (result.status === "rejected") {
    console.warn("[dweis] billing overview request failed", { label, error: errorMessage(result.reason) })
...
async function getAllCreditUsages(signal?: AbortSignal): Promise<CreditUsages> {
  ...
  while (nextToken && pageCount < billingCreditUsagesMaxPages) {
    if (seenTokens.has(nextToken)) {
      console.warn("[dweis] stopped billing balance pagination after repeated token")
      break
    }
...
export async function getBillingOverview(
  ...
  // user's personal account, matching Console and the identity used by DWeis's built-in model calls.
