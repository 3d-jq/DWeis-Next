...
export function SidebarSegmentControl({
  ...
  const t = useT()
  const options: Array<{ label: string; title: string; value: SidebarSegment }> = [
    { label: t("sidebar.segmentTasks"), title: t("sidebar.segmentTasksTitle"), value: "tasks" },
    { label: t("sidebar.segmentProjects"), title: t("sidebar.segmentProjectsTitle"), value: "projects" },
  ]

  return (
    <div className="grid grid-cols-2 gap-0.5 rounded-full bg-sidebar-accent/80 p-0.5">
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          ...
          className={cn(
            "oo-text-control flex h-7 min-w-0 items-center justify-center gap-1 rounded-full px-2 text-sidebar-foreground/75 transition-colors",
            value === option.value && "bg-background text-foreground shadow-sm",
          )}
          aria-pressed={value === option.value}
          onClick={() => onChange(option.value)}
        >
          {/* Work=公文包（办公任务）、Code=代码符号（编码项目），图标与语义对等 */}
          {option.value === "tasks" ? <Briefcase className="size-3.5" /> : <Code2 className="size-3.5" />}
