...
export function useComposerNavigation({
  ...
  }, [activeSession, draftProjectId, lastProjectId, route, startNewSessionDraft])
  ...
  const handleSelectComposerProject = React.useCallback(
    ...
    [activeChatSessionId, assignSessionProject, isDraftSession, setDraftProjectId, setIsDraftSession, setRoute, t],
  )
  ...
  const handleSelectProjectDirectory = React.useCallback(
    async (source: ProjectSelectionSource): Promise<void> => {
      releaseTransientFocus()
      const picker = globalThis.dweisnext?.selectProjectDirectory
      if (!picker) {
  ...
  const handleSelectSession = React.useCallback(
    ...
    [setDraftProjectId, setIsDraftSession, setPendingChatTransition, setRoute, setSelectedSessionId],
  )
