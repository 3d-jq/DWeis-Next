...
describe("sidebar persistence", () => {
  ...
  test("stores task sort mode with recent activity as the fallback", () => {
    ...
    storage.setItem("dweis.taskSortMode", "invalid")
    expect(readStoredTaskSortMode(storage)).toBe("updatedAt")
  })
  ...
  test("reads, writes, removes, and prunes collapsed project ids", () => {
    const storage = new MemoryStorage()
    const key = "dweis.projectSidebarCollapsed:account-a:team:team-id"
  ...
  test("migrates legacy organization-scoped collapsed project ids", () => {
    const storage = new MemoryStorage()
    const key = "dweis.projectSidebarCollapsed:account-a:team:team-id"
    const legacyKey = "dweis.projectSidebarCollapsed:account-a:organization:team-id"
