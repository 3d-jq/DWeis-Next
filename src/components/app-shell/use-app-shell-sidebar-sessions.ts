...
export function useAppShellSidebarSessions({
  ...
  return {
    ...
    // 两个 segment 都展示当前 persona 的全部会话（任务+项目两类）：
    // Work 选项目建的 projectSession、Code 不选项目建的 taskSession 都不能在
    // 当前视图消失——否则自动选中 effect 会把 selectedSessionId 重置为 null，
    // 聊天区落回欢迎空态（"进不去会话"严重 bug）。
    selectableSessions:
      sidebarSegment === "projects"
        ? [...selectableProjectSessions, ...selectableTaskSessions]
        : [...selectableTaskSessions, ...selectableProjectSessions],
    taskGroups,
  }
}
