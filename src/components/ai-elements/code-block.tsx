...
export const CodeBlockContent = ({
  ...
  // 流式降载：代码稳定（250ms 无变化）后才请求 shiki 高亮；增长期间只渲染纯文本。
  // 之前每帧对整段中间态全文 tokenize（长回答 O(n²)）+ LRU 缓存被中间态污染（命中率≈0），
  // 且 syncTokens useMemo 与 useEffect 各触发一次高亮。现在只保留一条异步高亮路径。
  const [debouncedCode, setDebouncedCode] = useState<string | null>(null)
  ...
  useEffect(() => {
    if (debouncedCode === null) {
      return
    }
    setTokenized(null)
    let cancelled = false

    const highlighted = highlightCode(debouncedCode, normalizedLanguage, (result) => {
      if (!cancelled) {
        setTokenized(result)
      }
    })
  ...
  }, [debouncedCode, normalizedLanguage])

  const tokens = tokenized ?? rawTokens

  return (
    <div className="relative overflow-auto bg-background">
      <CodeBlockBody showLineNumbers={showLineNumbers} tokenized={tokens} />
    </div>
  )
}
