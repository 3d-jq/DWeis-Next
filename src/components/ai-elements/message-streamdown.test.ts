...
describe("messageStreamdownControls", () => {
  ...
  it("routes Mermaid controls to the DWeis renderer and disables the native fullscreen portal", () => {
  ...
  it("keeps Mermaid fences on the dedicated DWeis renderer", () => {
...
describe("messageStreamdownLinkSafety", () => {
  ...
  it("opens encoded local paths through the trusted local-file service", async () => {
    ...
    // 全局桥名与 branding.windowBridge（"dweisnext"）一致，见 electron/preload.ts。
    vi.stubGlobal("dweisnext", { writeClipboardText: nativeWriteText })
    const browserWriteText = vi.fn(async () => undefined)
    Object.defineProperty(navigator, "clipboard", {
      configurable: true,
      value: { writeText: browserWriteText },
    })
    const modal = await renderLinkSafetyModal("/Users/me/Library/Application%20Support/dweis/report.html")
    const copyButton = [...document.querySelectorAll("button")].find((button) =>
    ...
    expect(document.body.textContent).toContain("/Users/me/Library/Application Support/dweis/report.html")

    await act(async () => copyButton?.click())
    expect(nativeWriteText).toHaveBeenCalledWith("/Users/me/Library/Application Support/dweis/report.html")
    expect(browserWriteText).not.toHaveBeenCalled()

    await act(async () => openButton?.click())
    expect(modal.invoke).toHaveBeenCalledWith("openLocalPath", {
      path: "/Users/me/Library/Application Support/dweis/report.html",
    })
