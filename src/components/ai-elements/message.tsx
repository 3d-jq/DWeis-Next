...
// Mermaid uses its dedicated renderer; ordinary fenced content uses DWeis's AI Elements code block.
  ...
  ({
    ...
    const responseChildren =
      typeof sourceChildren === "string"
        ...
        : sourceChildren
    // 流式平滑展示中（文本还在增长）：末尾显示闪烁光标（借鉴 AICSS StreamingText caret）。
    const streaming = smooth && typeof children === "string" && visibleChildren.length < children.length
    ...
    return (
      ...
      <Suspense fallback={<div className={cn("size-full whitespace-pre-wrap", className)}>{responseChildren}</div>}>
        <>
          <Streamdown
            className={cn(
              "oo-message-response size-full [&>*:first-child]:mt-0 [&>*:last-child]:mb-0",
              streaming && "oo-streaming-caret",
              className,
            )}
            components={{
