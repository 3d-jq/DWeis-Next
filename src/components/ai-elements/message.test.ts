...
describe("normalizeSingleLocalPathCodeFences", () => {
  it("turns a path-only fenced block into inline code", () => {
    expect(
      normalizeSingleLocalPathCodeFences(
        ["文件路径：", "```", "/Users/me/Library/Application Support/dweis/agent/artifacts/turn/image.png", "```"].join(
          "\n",
        ),
      ),
    ).toBe("文件路径：\n`/Users/me/Library/Application Support/dweis/agent/artifacts/turn/image.png`")
  })
...
describe("normalizeLocalImageMarkdown", () => {
  it("wraps a local image destination containing spaces in angle brackets", () => {
    const path =
      "/Users/me/Library/Application Support/dweis/agent/artifacts/ses_example/1783833651476-turn/mucha-corgi.png"
...
describe("compactLocalPath", () => {
  ...
  it("middle-truncates long local paths", () => {
    expect(compactLocalPath("/Users/me/Library/Application Support/dweis/agent/artifacts/turn/image.png", 32)).toBe(
...
describe("MarkdownImage", () => {
  it("prefers streamed resource URLs for local image previews", () => {
    expect(
      attachmentPreviewSource({
        ...
        resourceUrl: "dweis-artifact://resource/image",
      }),
    ).toBe("dweis-artifact://resource/image")
  ...
  it("decodes percent-encoded local paths from markdown image URLs", () => {
    expect(localImagePathFromSrc("/Users/me/Library/Application%20Support/dweis/agent/artifacts/turn/001.png")).toBe(
      "/Users/me/Library/Application Support/dweis/agent/artifacts/turn/001.png",
    )
  })
...
describe("nextSmoothedText", () => {
  it("reveals a prefix incrementally", () => {
    expect(nextSmoothedText("", "DWeis 正在处理一段较长的回复")).toBe("DWe")
  })
