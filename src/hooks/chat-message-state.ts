...
export function setMessageInfo(msgs: ChatMessage[], event: MessageStartedEvent): ChatMessage[] {
  ...
  return ensured.map((message) => {
    ...
    // message.updated 的 info.tokens 到达即更新，上下文占用指示器实时刷新（对齐 opencode footer）。
    const tokenUsage = event.tokenUsage ?? message.tokenUsage
    if (
...
/**
 * 首次插入文字类 part（text/reasoning）时对齐 opencode 最终顺序：文字始终在工具调用之前。
 * live 流式时 tool 事件可能早于 text 到达（settle 窗口不够），若按到达顺序追加会得到
 * [工具, 文字]，消息完成 reload 后又按 opencode 顺序重排成 [文字, 工具] → 顺序跳变。
 */
function upsertTextPartBeforeTools(parts: ChatMessagePart[], part: ChatMessagePart): ChatMessagePart[] {
  const existing = parts.findIndex((candidate) => candidate.partId === part.partId)
  if (existing !== -1) {
    const next = parts.slice()
    next[existing] = { ...next[existing], ...part }
    return next
  }
  const firstToolIndex = parts.findIndex((candidate) => candidate.kind === "tool")
  if (firstToolIndex === -1) {
    return [...parts, part]
  }
  const next = parts.slice()
  next.splice(firstToolIndex, 0, part)
  return next
}
...
export function setTextPart(msgs: ChatMessage[], event: MessageDeltaEvent): ChatMessage[] {
  ...
  const textPart: ChatMessagePart = { kind: "text", partId: event.partId, text }
  const next = ensured.slice()
  next[messageIndex] = {
    ...message,
    parts: message.role === "assistant" ? upsertTextPartBeforeTools(parts, textPart) : upsertPart(parts, textPart),
  }
  return next
}
...
export function setReasoningPart(msgs: ChatMessage[], event: MessageReasoningDeltaEvent): ChatMessage[] {
  ...
  next[messageIndex] = {
    ...message,
    parts: upsertTextPartBeforeTools(message.parts, { kind: "reasoning", partId: event.partId, text }),
  }
  return next
}
