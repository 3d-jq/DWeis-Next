...
function patchUpdateError(error: unknown): AppUpdateState {
  ...
  const next: AppUpdateState = current
    ...
    : {
        channel: "stable",
        currentVersion: globalThis.dweisnext?.version ?? "—",
        isPackaged: true,
...
export function useAppUpdate(): UseAppUpdate {
  ...
  const check = React.useCallback(async (): Promise<AppUpdateState | null> => {
    try {
      ...
      console.error("[dweis] checkForAppUpdate failed:", error)
  ...
  const install = React.useCallback(async (): Promise<void> => {
    ...
    await service.invoke("installDownloadedAppUpdate").catch((error: unknown) => {
      ...
      console.error("[dweis] installDownloadedAppUpdate failed:", error)
  ...
  const setChannel = React.useCallback(
    async (channel: UpdateChannel) => {
      try {
        ...
        console.error("[dweis] setUpdateChannel failed:", error)
