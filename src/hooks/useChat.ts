...
export interface UseChat {
  ...
  runShellCommand: (sessionId: string, command: string) => Promise<void>
...
export function useChat(activeSessionId: string | null, activeRunsRefreshKey?: string): UseChat {
  ...
  const setPermissionMode = React.useCallback(
    async (sessionId: string, mode: AgentPermissionMode): Promise<void> => {
      ...
      try {
        ...
        console.error("[dweis] set chat permission mode failed", err)
  ...
  const reload = React.useCallback(
    async (sessionId: string): Promise<ChatMessage[] | null> => {
      ...
      try {
        ...
        console.error("[dweis] getMessages failed", err)
  ...
  React.useEffect(() => {
    const offs = [
      ...
      chatService.serverEvents.on("assistantActivity", (e) => {
        if (e.phase === "compacting" || e.phase === "resuming") {
          // 上下文压缩：只驱动对话流分隔线，不改会话 status —— 否则 streaming 会让
          // turnInFlight/activeAssistantMessageId 激活压缩点前已完成的回合，
          // 把「已处理」改成「处理中」并重新开始计时（对齐 opencode：压缩不动前面回合）。
      ...
      chatService.serverEvents.on("messageCompleted", (e) => {
        removeAnsweredPendingQuestions(e.sessionId)
        flushPendingToolParts()
        // 提问回合：消息虽已完成但还在等用户回答（question 工具 pending），保持"处理中"
        // 不恢复 ready——否则用户回答后回合会从"已处理"跳变回"处理中"，显得像凭空多出一个处理中。
        const stillWaitingForQuestion = (pendingQuestionsMapRef.current[e.sessionId] ?? []).length > 0
        setStatus(e.sessionId, stillWaitingForQuestion ? "streaming" : "ready")
        setActivity(e.sessionId, undefined)
  ...
  React.useEffect(() => {
    ...
    void chatService
      ...
      .catch((err: unknown) => {
        console.error("[dweis] getActiveRuns failed", err)
  ...
  React.useEffect(() => {
    ...
    void chatService
      ...
      .catch((err: unknown) => {
        ...
        console.error("[dweis] getSessionSnapshot failed", err)
  ...
  const compact = React.useCallback(
    async (sessionId: string) => {
      ...
      clearSessionError(sessionId)
      try {
        await chatService.invoke("compactSession", sessionId)
      } catch (err) {
        reportRendererHandledError("chat", "compactSession invoke failed", err)
        setSessionError(sessionId, err instanceof Error ? err.message : String(err))
        throw err
      }
    },
    [chatService, clearSessionError, setGlobalError, setSessionError],
  )
  ...
  const undo = React.useCallback(
    async (sessionId: string) => {
      ...
      clearSessionError(sessionId)
      try {
        await chatService.invoke("undoSession", sessionId)
      } catch (err) {
        reportRendererHandledError("chat", "undoSession invoke failed", err)
      }
    },
    [chatService, clearSessionError, setGlobalError],
  )
  ...
  const redo = React.useCallback(
    async (sessionId: string) => {
      ...
      clearSessionError(sessionId)
      try {
        await chatService.invoke("redoSession", sessionId)
      } catch (err) {
        reportRendererHandledError("chat", "redoSession invoke failed", err)
      }
    },
    [chatService, clearSessionError, setGlobalError],
  )
  ...
  const runShellCommand = React.useCallback(
    async (sessionId: string, command: string) => {
      ...
      clearSessionError(sessionId)
      try {
        await chatService.invoke("runShellCommand", sessionId, command)
      } catch (err) {
        reportRendererHandledError("chat", "runShellCommand invoke failed", err)
      }
    },
    [chatService, clearSessionError, setGlobalError],
  )
