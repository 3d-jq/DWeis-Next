...
test("mergePathValues preserves priority while removing empty and duplicate entries", () => {
  assert.equal(
    mergePathValues([
      ["/dweis/bin", "/usr/bin"].join(path.delimiter),
      ["/opt/homebrew/bin", "/usr/bin", ""].join(path.delimiter),
    ]),
    ["/dweis/bin", "/usr/bin", "/opt/homebrew/bin"].join(path.delimiter),
  )
})

test("resolveUserCommandPath puts DWeis binaries before the login shell and fallback paths", async () => {
  const result = await resolveUserCommandPath({
    env: { HOME: "/Users/test", PATH: ["/usr/bin", "/bin"].join(":"), SHELL: "/bin/zsh" },
    platform: "darwin",
    preferredDirectories: ["/Applications/DWeis.app/Contents/Resources/bin"],
    shellPathReader: async () => ["/opt/homebrew/bin", "/usr/bin"].join(":"),
  })

  assert.deepEqual(result.split(":"), [
    "/Applications/DWeis.app/Contents/Resources/bin",
    "/opt/homebrew/bin",
    "/usr/bin",
    "/bin",
    "/usr/local/bin",
...
test("resolveUserCommandPath falls back without blocking when login shell PATH is unavailable", async () => {
  const result = await resolveUserCommandPath({
    ...
    platform: "darwin",
    preferredDirectories: ["/dweis/bin"],
...
test("resolveUserCommandPath does not invoke a Unix login shell on Windows", async () => {
  ...
  const result = await resolveUserCommandPath({
    ...
    platform: "win32",
    preferredDirectories: ["C:\\DWeis\\bin"],
