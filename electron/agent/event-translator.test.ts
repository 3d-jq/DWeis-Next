...
test("message.updated forwards assistant token usage for realtime context meter", () => {
  const out = translateOpencodeEvent({
    type: "message.updated",
    properties: {
      info: {
        ...
        sessionID: "s1",
        role: "assistant",
        tokens: { total: 100, input: 60, output: 40, reasoning: 0, cache: { read: 5, write: 0 } },
      },
    },
  })
  ...
  assert.deepEqual(out, [
    {
      ...
      data: {
        ...
        messageId: "m1",
        role: "assistant",
        tokenUsage: { total: 100, input: 60, output: 40, reasoning: 0, cache: { read: 5, write: 0 } },
      },
    },
  ])
})

test("message.updated omits tokenUsage while streaming tokens are still all-zero", () => {
  // 流式中间态 tokens 未到齐：全 0 时不得带 tokenUsage，否则渲染层会把消息
  // tokenUsage 覆盖为全 0，上下文占用指示器在回答期间显示 0。
  const out = translateOpencodeEvent({
    type: "message.updated",
    properties: {
      info: {
...
test("text part.updated treats DWeis internal metadata as synthetic", () => {
  const out = translateOpencodeEvent({
    type: "message.part.updated",
    properties: {
      part: {
        id: "p1",
        messageID: "m1",
        metadata: { dweisVisibility: "internal" },
        sessionID: "s1",
...
test("reasoning part.delta streams incrementally after part.updated", () => {
  translateOpencodeEvent({
    type: "message.part.updated",
    properties: { part: { id: "r2", sessionID: "s1", messageID: "m1", type: "reasoning", text: "" } },
  })
  const out = translateOpencodeEvent({
    type: "message.part.delta",
    properties: { sessionID: "s1", messageID: "m1", partID: "r2", field: "text", delta: "逐步排查" },
  })
  assert.deepEqual(out, [
    {
      event: "messageReasoningDelta",
      data: { sessionId: "s1", messageId: "m1", partId: "r2", text: "", delta: "逐步排查" },
    },
  ])
})

test("text part.delta streams incrementally after part.updated", () => {
  translateOpencodeEvent({
    type: "message.part.updated",
    properties: { part: { id: "t1", sessionID: "s1", messageID: "m1", type: "text", text: "" } },
  })
  const out = translateOpencodeEvent({
    type: "message.part.delta",
    properties: { sessionID: "s1", messageID: "m1", partID: "t1", field: "text", delta: "你好" },
  })
  assert.deepEqual(out, [
    { event: "messageDelta", data: { sessionId: "s1", messageId: "m1", partId: "t1", text: "", delta: "你好" } },
  ])
})

test("part.delta without a prior part.updated is dropped (type unknown)", () => {
  const out = translateOpencodeEvent({
    type: "message.part.delta",
    properties: { sessionID: "s1", messageID: "m1", partID: "orphan-1", field: "text", delta: "x" },
  })
  assert.deepEqual(out, [])
})

test("part.delta with a non-text field is dropped", () => {
  translateOpencodeEvent({
    type: "message.part.updated",
    properties: { part: { id: "r4", sessionID: "s1", messageID: "m1", type: "reasoning", text: "" } },
  })
  const out = translateOpencodeEvent({
    type: "message.part.delta",
    properties: { sessionID: "s1", messageID: "m1", partID: "r4", field: "state", delta: "{}" },
  })
  assert.deepEqual(out, [])
})

test("part.removed clears the type cache so later deltas are dropped", () => {
  translateOpencodeEvent({
    type: "message.part.updated",
    properties: { part: { id: "r3", sessionID: "s1", messageID: "m1", type: "reasoning", text: "" } },
  })
  translateOpencodeEvent({
    type: "message.part.removed",
    properties: { sessionID: "s1", messageID: "m1", partID: "r3" },
  })
  const out = translateOpencodeEvent({
    type: "message.part.delta",
    properties: { sessionID: "s1", messageID: "m1", partID: "r3", field: "text", delta: "x" },
  })
  assert.deepEqual(out, [])
})
...
test("normalizeMessage hides DWeis internal attachment context from user history", () => {
  assert.deepEqual(
    normalizeMessage({
      info: { id: "m1", role: "user", time: { created: 1 } },
      parts: [
...
test("normalizeMessage hides compaction checkpoints, summaries, and synthetic continuations", () => {
  ...
  // 压缩总结消息：不渲染（internal），但保留 tokenUsage 供上下文占用计算（对齐 opencode footer）。
  const summary = normalizeMessage({
    info: {
      id: "compact-summary",
      role: "assistant",
      summary: true,
      time: { created: 2 },
      tokens: { total: 100, input: 60, output: 40, reasoning: 0, cache: { read: 0, write: 0 } },
    },
  ...
  assert.deepEqual(summary, {
    ...
    parts: [],
    createdAt: 2,
    tokenUsage: { total: 100, input: 60, output: 40, reasoning: 0, cache: { read: 0, write: 0 } },
  })
  assert.equal(
    normalizeMessage({
