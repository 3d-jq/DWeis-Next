...
test("isPureOoCliCommand allows single oo CLI invocations", () => {
  ...
  assert.equal(isPureOoCliCommand('"$DWEIS_OO_BIN" version --json'), true)
  assert.equal(isPureOoCliCommand("${DWEIS_OO_BIN} connector schema metaso.search --json"), true)
})

test("isPureOoCliCommand rejects shell composition around oo", () => {
  assert.equal(isPureOoCliCommand('oo search "metaso" --json && rm -rf /tmp/dweis-test'), false)
