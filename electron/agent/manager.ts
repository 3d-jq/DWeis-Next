...
export function buildArtifactSystem(artifactDir: string | undefined, outputProjectRoot?: string): string | undefined {
  ...
  return [
    ...
    "- When the final deliverable is one to four image files and inline viewing helps the user, include Markdown image references in the final response using their absolute local paths (no angle brackets), for example ![short title](C:/absolute/path/image.png) or ![short title](/absolute/path/image.png).",
