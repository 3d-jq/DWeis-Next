...
describe("runtime tool assembly", () => {
  ...
  it("always ships user-configurable tools for hot-add (enablement read from config file at call time)", () => {
    const enabled = agentToolFiles(true)
    ...
    for (const connectors of [true, false]) {
      const files = agentToolFiles(connectors)
      expect(files["generate_image.ts"]).toBeDefined()
      expect(files["dweis_websearch.ts"]).toBeDefined()
    }
  })
})
...
afterEach(() => {
  ...
  delete process.env.DWEIS_CONSOLE_URL
  delete process.env.DWEIS_CONNECTOR_URL
  delete process.env.DWEIS_LINK_RUNTIME
  delete process.env.DWEIS_TEAM_NAME
  delete process.env.DWEIS_TEAM_SCOPE_PATH
  delete process.env.DWEIS_ORGANIZATION_NAME
  delete process.env.DWEIS_ORGANIZATION_SCOPE_PATH
...
beforeEach(() => {
  process.env.DWEIS_TEAM_NAME = "team-a"
})

describe("list_apps embedded runtime", () => {
  it("uses endpoint identity without organization arguments for OpenConnector", async () => {
    process.env.DWEIS_LINK_RUNTIME = "openconnector"
    process.env.DWEIS_CONNECTOR_URL = "http://127.0.0.1:3000"
    delete process.env.DWEIS_TEAM_NAME
  ...
  it("reads legacy organization environment variables during migration", async () => {
    delete process.env.DWEIS_TEAM_NAME
    process.env.DWEIS_ORGANIZATION_NAME = "legacy-team"
    process.env.DWEIS_ORGANIZATION_SCOPE_PATH = "/tmp/legacy-scope.json"
    const runtime = loadListAppsTool(
  ...
  it("keeps team identity in structured inventory errors", async () => {
    process.env.DWEIS_TEAM_SCOPE_PATH = "/tmp/team-scope.json"
    const commands: string[][] = []
    const runtime = loadListAppsTool(
      async (...args) => {
        commands.push(args[1] as string[])
  ...
  it("fails closed when the session workspace file is unreadable", async () => {
    process.env.DWEIS_TEAM_SCOPE_PATH = "/tmp/team-scope.json"
    process.env.DWEIS_TEAM_NAME = "stale-default"
    let calls = 0
    const runtime = loadListAppsTool(
      async () => {
        calls += 1
...
describe("call_action embedded runtime", () => {
  it("builds OpenConnector authorization URLs from the configured Console origin", async () => {
    process.env.DWEIS_LINK_RUNTIME = "openconnector"
    process.env.DWEIS_CONNECTOR_URL = "http://127.0.0.1:3000"
    process.env.DWEIS_CONSOLE_URL = "http://127.0.0.1:5173"
  ...
  it("runs one canary and skips matching queued calls after an authorization block", async () => {
    process.env.DWEIS_CONSOLE_URL = "https://console.example.test"
    let calls = 0
    const runtime = loadCallActionTool(async () => {
      calls += 1
      const error = new Error("connector failed") as Error & { stderr: string }
