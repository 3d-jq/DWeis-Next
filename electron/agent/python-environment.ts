/** Private Python virtual environment created by DWeis for one task. */
export const DWEIS_MANAGED_PYTHON_ENV_DIRNAME = ".wanta-python"
...
export function managedPythonEnvironmentPath(processDir: string): string {
  return joinPath(processDir, DWEIS_MANAGED_PYTHON_ENV_DIRNAME)
}
...
export function isManagedPythonExecutable(executable: string): boolean {
  ...
  return (
    normalized.endsWith(`/${DWEIS_MANAGED_PYTHON_ENV_DIRNAME}/bin/python`) ||
    normalized.endsWith(`/${DWEIS_MANAGED_PYTHON_ENV_DIRNAME}/bin/python3`) ||
    normalized.endsWith(`/${DWEIS_MANAGED_PYTHON_ENV_DIRNAME}/scripts/python.exe`)
  )
}
