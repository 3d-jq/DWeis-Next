...
function isOoExecutable(word: string): boolean {
  return word === "oo" || word === "$DWEIS_OO_BIN" || word === "${DWEIS_OO_BIN}"
}
...
const ooCommandSegment = /(?:^|[;&|]{1,2}\s*)(?:oo|"?\$DWEIS_OO_BIN"?|"?\$\{DWEIS_OO_BIN\}"?)(?:\s|$)/u
const forbiddenOoMutation =
  /(?:^|[;&|]{1,2}\s*)(?:oo|"?\$DWEIS_OO_BIN"?|"?\$\{DWEIS_OO_BIN\}"?)\s+(?:(?:auth|login|logout|config)(?:\s|[;&|]|$)|connector\s+(?:login|logout)(?:\s|[;&|]|$))/u
...
const unsupportedWrapperSyntax = /(?:`|\$(?!(?:DWEIS_OO_BIN\b|\{DWEIS_OO_BIN\}))|%[^%\s]+%|![^!\s]+!)/u
