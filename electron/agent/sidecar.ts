...
export interface SidecarOptions {
  ...
  config: Config
  /** 额外注入到 opencode 进程的环境变量（OO_ 系列 / DWEIS_ 系列 / PATH 等）。 */
...
export class OpencodeSidecar {
  ...
  private async reap(proc: ChildProcessWithoutNullStreams, client: OpencodeClient | null): Promise<void> {
    ...
    try {
      ...
      console.warn("[dweis] failed to reap opencode process tree:", error)
...
function attachStreamDiagnostics(stream: Readable, source: "stdout" | "stderr"): () => void {
  ...
  const flush = (): void => {
    ...
    if (source === "stderr") {
      console.warn("[dweis] opencode stderr batch:", snapshot)
    }
