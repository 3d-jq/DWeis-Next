...
/**
 * message.part.delta 只携带 {sessionID, messageID, partID, field, delta}，不含 part 类型；
 * 用 message.part.updated（全量 part）建立 partID → type 缓存，供 delta 路由到 text/reasoning 流式。
 * partID 全局递增唯一；message.part.removed 时清除，避免随会话累积。
 */
...
export function translateOpencodeEvent(event: OpencodeEvent): ChatEmit[] {
  ...
  switch (event.type) {
    ...
    case "message.updated": {
      ...
      // 对齐 opencode footer：message.updated 的 info.tokens 到达即推送，
      // 渲染层据此实时刷新上下文占用（不必等 reload）。
      const tokenUsage = messageTokenUsage(info)
    ...
    case "message.part.delta": {
      // opencode 的 text/reasoning 流式增量走 message.part.delta（updatePartDelta），
      // 全量 part 只在 start/end 经 message.part.updated 到达；不处理则两者都"结束时全量显示"。
      const p = props as { sessionID?: string; messageID?: string; partID?: string; field?: string; delta?: string }
      if (!p.sessionID || !p.messageID || !p.partID || p.field !== "text" || typeof p.delta !== "string") {
        return []
      }
      const type = partTypeByPartId.get(p.partID)
      if (type === "reasoning") {
        return [
          {
    ...
    case "message.part.removed": {
      ...
      if (p.partID) {
        partTypeByPartId.delete(p.partID)
      }
      return [
        {
...
function isInternalUserTextPart(part: OpencodePart): boolean {
  return part.synthetic === true || part.metadata?.dweisVisibility === "internal"
}
...
export function normalizeMessage(message: { info?: unknown; parts?: unknown }): ChatMessage | null {
  ...
  if (info.summary === true) {
    // 压缩总结消息：不渲染内容（internal），但保留该轮 prompt 实测 tokenUsage，
    // 供渲染层上下文占用指示器压缩后立即同步（对齐 opencode footer 直接显示 summary tokens）。
    const tokenUsage = messageTokenUsage(info)
    return {
      id: info.id,
      internal: true,
