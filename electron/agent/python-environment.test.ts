...
test("managed Python environment paths stay inside the task process directory", () => {
  const processDir = "/tmp/dweis-process/task-1"

  assert.equal(managedPythonEnvironmentPath(processDir), "/tmp/dweis-process/task-1/.wanta-python")
  assert.equal(managedPythonExecutable(processDir, "darwin"), "/tmp/dweis-process/task-1/.wanta-python/bin/python")
  assert.equal(
    managedPythonExecutable(processDir, "win32"),
    "/tmp/dweis-process/task-1/.wanta-python/Scripts/python.exe",
  )
  assert.equal(
    managedPythonExecutable("C:\\Users\\me\\AppData\\Local\\dweis\\process\\task-1", "win32"),
    "C:/Users/me/AppData/Local/dweis/process/task-1/.wanta-python/Scripts/python.exe",
  )
  assert.deepEqual(managedPythonExecutables(processDir, "linux"), [
    "/tmp/dweis-process/task-1/.wanta-python/bin/python",
    "/tmp/dweis-process/task-1/.wanta-python/bin/python3",
  ])
})
