...
const unreadableImportMessage =
  "DWEIS_KNOWLEDGE_IMPORT_UNREADABLE: The selected WikiGraph file could not be imported because DWeis cannot make the managed copy readable with the current WikiGraph SDK. The original file was not modified."
...
async function removeUnreadableImportedArchive(imported: WikiGraphLibraryArchiveRecord): Promise<void> {
  await removeWikiGraphLibraryArchiveWithSDK({ target: requireLibraryTarget(imported.uri) }).catch(
    (cleanupError: unknown) => {
      console.warn("[dweis] failed to clean unreadable WikiGraph import:", cleanupError)
    },
  )
}
