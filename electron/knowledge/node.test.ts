...
describe("KnowledgeServiceImpl", () => {
  ...
  it("downgrades inspect and index SDK failures while preserving metadata summary", async () => {
    ...
    const service = new KnowledgeServiceImpl({
      runtime: { managedLibraryDir: "/tmp/dweis/library", stateDir: "/tmp/dweis/state" },
    })
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith("[dweis] failed to inspect knowledge base:", expect.any(Error))
    } finally {
      warn.mockRestore()
    }
  })
  ...
  it("prepares the archive before reading summary details", async () => {
    ...
    expect(runner.prepareWikiGraphArchive).toHaveBeenCalledWith(
      { managedLibraryDir: "/tmp/dweis/library", stateDir: "/tmp/dweis/state" },
      "public-id",
    )
  ...
  it("keeps listing other knowledge bases when one archive cannot be prepared", async () => {
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith("[dweis] failed to prepare knowledge base:", expect.any(Error))
    } finally {
      warn.mockRestore()
    }
  })
