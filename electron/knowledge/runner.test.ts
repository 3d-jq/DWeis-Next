...
describe("WikiGraph SDK adapter", () => {
  it("uses SDK stateDir and binds the default library to DWeis managed storage", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "dweis-wg-adapter-"))
  ...
  it("lists default library archives using SDK publicId as the DWeis-facing id", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "dweis-wg-adapter-"))
  ...
  it("fails and removes the managed copy when post-upgrade validation cannot read chapters", async () => {
    ...
    expect((importError as Error).message).toContain("DWEIS_KNOWLEDGE_IMPORT_UNREADABLE")
