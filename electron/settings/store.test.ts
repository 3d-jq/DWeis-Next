...
test("SettingsStore round-trips persisted settings", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "dweis-settings-"))
  const store = new SettingsStore(dir)
  assert.deepEqual(store.read(), {})
  store.write({ browserEnabled: false, themeSource: "dark" })
  assert.deepEqual(store.read(), { browserEnabled: false, themeSource: "dark" })
