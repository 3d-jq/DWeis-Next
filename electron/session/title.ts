...
export function sanitizeGeneratedSessionTitle(
  ...
  // MiniMax 等模型把思考内联在输出开头（<think>…</think>，从不发 reasoning_content）：
  // 标题提取前剥离，否则第一行思考内容会直接成为会话标题（如标题变成 "think" 或推理文本）。
  const title = titleFromGeneratedOutput(stripInlineThink(raw))
...
const inlineThinkPattern = /<think>[\s\S]*?(?:<\/think>|$)/g

/** 剥离模型输出开头的内联思考块（含未闭合的流式中形态），返回剩余文本。 */
function stripInlineThink(value: string): string {
  return value.replace(inlineThinkPattern, " ").replace(/\s+/g, " ").trim()
}
