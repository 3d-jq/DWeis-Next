...
interface SessionServiceDeps {
  ...
  /** 当前人群模式（work/code）：项目列表按此过滤、新建项目按此归属。 */
  getPersona?: () => Persona
}
  ...
  implements IConnectionService<SessionService>
{
  ...
  /** 查询会话归属模式（不按当前模式过滤，供通知路由等跨模式场景使用）。 */
  public async sessionPersona(sessionId: string): Promise<Persona | undefined> {
  ...
  public async listProjects(req: SessionScopeRequest): Promise<SessionProject[]> {    const agent = this.agent
    ...
    const persona = this.deps.getPersona?.() ?? "work"
    return [...this.projects.values()]
      .filter((project) => !project.archivedAt)
      .filter((project) => sessionScopeMatches(project.scope, requestedScope))
      .filter((project) => (project.persona ?? "work") === persona)
      .sort((a, b) => {
  ...
  private async createMutation(req: CreateSessionRequest, revision: number): Promise<SessionInfo> {
    ...
    return { ...info, scope, persona, ...(scopedProjectId ? { projectId: scopedProjectId } : {}) }
  }
  ...
  private async assignSessionProjectMutation(req: AssignSessionProjectRequest, revision: number): Promise<void> {
    ...
    const sessionPersona = current.persona
    ...
    if (canAssign) {
      ...
      if (sessionPersona === undefined) {
        next.persona = project.persona ?? "work"
      }
    } else {
  ...
  private async resolveSession(id: string, visibility: "active" | "archived"): Promise<SessionInfo | null> {
    ...
    const persona = this.resolveSessionPersona(metadata, scope)
  ...
  private mergeLocalState(
    ...
    const currentPersona = this.deps.getPersona?.() ?? "work"
    return sessions
      .map((session) => {
        const usedAt = this.sessionActivityAt.get(session.id)
        const metadata = this.sessionMetadata.get(session.id)
      ...
      .filter((session) => (session.persona ?? "work") === currentPersona)
  ...
  /**
   * 会话归属模式：metadata.persona（创建时写入）优先；旧会话（无 persona）从关联项目
   * 继承（查项目原始 persona，不被当前模式掩蔽，scope 需匹配）；都无则默认 work。
   */
  private resolveSessionPersona(metadata: SessionMetadata | undefined, scope: SessionScope): Persona {
    if (metadata?.persona) {
  ...
  private resolveValidSessionProject(
    ...
    // 项目按人群模式独立：不属于当前模式的项目对会话表现为无项目（普通任务）。
    if ((project.persona ?? "work") !== (this.deps.getPersona?.() ?? "work")) {
      return undefined
    }
    return project
  }
  ...
  private logFailure(message: string, error: unknown, fields: Record<string, unknown> = {}): void {
    console.warn(`[dweis] ${message}:`, error)
