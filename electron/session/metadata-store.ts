...
export interface SessionMetadata {
  ...
  /** 会话归属人群模式（work/code）：创建时写入当前模式，列表按当前模式过滤。 */
  persona?: Persona
  pinnedAt?: number
  archivedAt?: number
}
...
function normalizePersona(value: unknown): Persona | undefined {
  return value === "code" || value === "work" ? value : undefined
}
...
function normalizeMetadata(value: unknown): Map<string, SessionMetadata> {
  ...
  for (const [id, entry] of Object.entries(record)) {
    ...
    const persona = normalizePersona(source.persona)
    if (persona) {
      next.persona = persona
    }
    // Do not restore legacy DWeis-owned knowledge registry references from disk. WikiGraph
...
function serializeMetadata(metadata: Map<string, SessionMetadata>): PersistedSessionMetadata {
  ...
  for (const [id, entry] of metadata.entries()) {
    ...
    const persona = normalizePersona(entry.persona)
    if (persona) {
      next.persona = persona
    }
  ...
  return { version: 6, sessions }
}

/** 会话展示元数据：置顶和归档属于 DWeis 侧边栏状态，不修改 OpenCode 会话本体。 */
