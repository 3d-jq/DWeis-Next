...
describe("session title helpers", () => {
  ...
  it("strips inline <think> blocks so reasoning never becomes the title", () => {
    expect(
      sanitizeGeneratedSessionTitle("<think>用户想了解武松打虎的故事背景，应该给出简洁的标题。</think>\n武松打虎", {
        text: "讲一下武松打虎",
      }),
    ).toEqual({ title: "武松打虎", usedFallback: false })
  })

  it("falls back when the generated output is only an unclosed <think> block", () => {
    expect(
      sanitizeGeneratedSessionTitle("<think>正在思考如何为这段对话起一个合适的标题", {
        text: "讲一下武松打虎",
      }),
    ).toEqual({ title: "讲一下武松打虎", usedFallback: true })
  })
