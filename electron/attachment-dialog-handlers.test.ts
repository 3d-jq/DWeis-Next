...
vi.mock("electron", () => ({
  app: { getPath: vi.fn(() => "/tmp/dweis-user-data") },
...
describe("prepareSelectedAttachment", () => {
  it("keeps the immutable snapshot when spreadsheet preview rejects", async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "dweis-attachment-handler-"))
...
describe("snapshotSelectedAttachment", () => {
  it("creates a private immutable copy without changing the source", async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "dweis-attachment-snapshot-"))
    temporaryDirectories.push(directory)
    const sourcePath = path.join(directory, "inventory.xlsx")
    ...
    // Windows 无 POSIX 权限位（chmod 0o400 → 0o444），仅 POSIX 检查只读位
  ...
  it("rejects a stale selection instead of falling back to the mutable source", async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "dweis-attachment-stale-"))
    temporaryDirectories.push(directory)
    const sourcePath = path.join(directory, "inventory.xlsx")
  ...
  it.each([".", ".."])("normalizes the reserved snapshot name %s", async (name) => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "dweis-attachment-reserved-name-"))
    temporaryDirectories.push(directory)
...
describe("releaseManagedAttachmentPaths", () => {
  it("deletes only a picker-trusted managed snapshot", async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "dweis-attachment-release-"))
    temporaryDirectories.push(directory)
