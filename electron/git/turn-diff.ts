...
/** 从 unified diff 文本解析增删行数（+/- 前缀行，排除 ---/+++ 文件头与 hunk 头）。 */
...
export function buildUnifiedDiff(
  ...
  // 只算一次 diff：createTwoFilesPatch 生成 patch，增删行数直接从 patch 文本解析，
  // 不再另跑一遍 diffLines 计数（同一内容两遍 Myers diff 是纯浪费的主线程 CPU）。
  ...
  const { additions, deletions } = countPatchLines(patchBody)
