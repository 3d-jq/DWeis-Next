...
test("collectGitTurnDiffs compares against dirty turn baseline instead of HEAD", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-turn-diff-"))
  try {
    await git(root, ["init"])
...
test("collectGitTurnDiffs never follows an untracked symbolic link", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-turn-diff-symlink-"))
  const outside = await mkdtemp(path.join(os.tmpdir(), "dweis-turn-diff-outside-"))
  try {
    await git(root, ["init"])
    await git(root, ["config", "user.email", "test@example.com"])
    await git(root, ["config", "user.name", "Test User"])
...
test("captureGitTurnBaseline scopes a repository snapshot to the selected subdirectory", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-turn-diff-scope-"))
  try {
    await git(root, ["init"])
    await git(root, ["config", "user.email", "test@example.com"])
    await git(root, ["config", "user.name", "Test User"])
...
test("collectGitTurnDiffs does not attribute an uncaptured initial edit to the agent", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-turn-diff-truncated-"))
  try {
    await git(root, ["init"])
    await git(root, ["config", "user.email", "test@example.com"])
    await git(root, ["config", "user.name", "Test User"])
