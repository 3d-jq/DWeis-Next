...
export interface AuthManagerDeps {
  store: AuthStore
  /** deep-link 协议（生产 dweis / dev dweis-local，见 branding）。 */
...
export class AuthManager {
  ...
  // DWeis Next runs in local self-managed mode only and has no cloud account,
  // so the renderer always observes an unauthenticated state. This keeps cloud
  // features (Teams, billing, model marketplace, sign-in) hidden even if a
  // stale OOMOL session cookie happens to exist on disk.
  public getAuthState(): Promise<AuthState> {
    return Promise.resolve({ status: "unauthenticated", updatedAt: new Date().toISOString() })
  }
  ...
  public async broadcastAuthState(): Promise<void> {
    // Local self-managed mode only: the renderer must always see an
    // unauthenticated state, regardless of any stored cloud session.
    const state: AuthState = { status: "unauthenticated", updatedAt: new Date().toISOString() }
  ...
  public async expireSession(): Promise<AuthState> {
    ...
    await this.runtime.clearCookies().catch((error: unknown) => {
      console.warn("[dweis] failed to clear expired session cookies:", error)
    ...
    await this.deps.applyAccount(null).catch((error: unknown) => {
      console.error("[dweis] failed to apply local runtime after session expiry:", error)
  ...
  public async logout(): Promise<AuthState> {
    ...
    await this.runtime.clearCookies().catch((error: unknown) => {
      console.warn("[dweis] failed to clear session cookies:", error)
      logDiagnostic("auth", "failed to clear session cookies", { error }, "warn")
    })
    // 无 profile 时也必须执行：磁盘状态可能已被外部清理，但内存中仍可能存在旧 sidecar。
    await this.deps.applyAccount(null).catch((error: unknown) => {
      console.error("[dweis] failed to apply local runtime after sign-out:", error)
  ...
  private async handleBrowserLoginCallback(request: {
    ...
    try {
      ...
      console.error("[dweis] browser sign-in failed:", wrapped)
  ...
  private async adoptAccount(account: AuthRuntimeAccount): Promise<AuthState> {
    ...
    await this.deps.applyAccount(account).catch((error: unknown) => {
      console.error("[dweis] failed to start agent after sign-in:", error)
