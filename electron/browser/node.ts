...
export interface BrowserServiceOptions {
  ...
  /**
   * 会话 id 归一化：子代理（task 子会话）调用浏览器工具时携带自己的 sessionID，
   * 渲染层只认父会话。映射为父会话 id 后，页面管理/事件广播/查询全部对齐聊天会话，
   * 否则子代理浏览时 browserRequested/stateChanged 被渲染层过滤，用户看不到画面。
   */
...
export class BrowserManager {
  ...
  private readonly normalizeSessionId: (sessionId: string) => string
  ...
  public constructor(options: BrowserServiceOptions) {
    ...
    this.normalizeSessionId = options.normalizeSessionId ?? ((sessionId) => sessionId)
  }
  ...
  public async execute(request: BrowserControlRequest, signal?: AbortSignal): Promise<BrowserControlResult> {
    // 子代理会话归一化为父会话：页面、事件、查询全部对齐聊天会话 id（见 BrowserServiceOptions.normalizeSessionId）。
    const sessionId = this.normalizeSessionId(request.sessionId)
    const page = await this.ensurePage(sessionId)
    this.events?.browserRequested(sessionId)
  ...
  private getPartitionSession(): Session {
    ...
    const partitionSession = session.fromPartition(`persist:dweis-browser-${scope}`, { cache: true })
