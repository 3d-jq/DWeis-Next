...
describe("createSpreadsheetAgentInput", () => {
  it("writes a private text attachment for an XLSX workbook", async () => {
    ...
    expect(result).toMatchObject({
      ...
      agentPath: path.join("/tmp/dweis-user-data", "attachments", "agent", "attachment-id-库存表-extracted.txt"),
    })
    expect(ensureDirectory).toHaveBeenCalledWith(path.join("/tmp/dweis-user-data", "attachments", "agent"))
