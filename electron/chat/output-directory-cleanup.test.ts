...
test("removeSessionOutputDirectories removes managed and project turn directories", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-output-cleanup-"))
  try {
    ...
    const projectArtifactRoot = path.join(root, "project", ".dweis", "artifacts", "session-1", turnName)
...
test("removeSessionOutputDirectories ignores persisted paths outside managed layouts", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-output-cleanup-boundary-"))
  try {
