...
function normalizeArtifactBundles(value: unknown): ArtifactBundles {
  ...
  for (const [sessionId, messages] of Object.entries(persisted.sessions)) {
    ...
    for (const [messageId, bundle] of Object.entries(messages)) {
      ...
      // 读取时清理历史双条目（恢复/物化重复归档的 -N 同名同大小文件）
      const items = deduplicatedArtifactItems(bundle.items)
      sessionRecords.set(messageId, {
        ...bundle,
        items,
        totalItems: Math.min(bundle.totalItems, items.length),
      })
    }
...
async function managedArtifactGroup(
  ...
  const visit = async (directory: string, depth: number): Promise<void> => {
    ...
    for (const entry of entries.sort((left, right) =>
      // 固定 "en" locale 使排序与系统 locale 无关（zh-CN 下 CJK 与保留名比较会不稳定）
...
/** 当前轮目录已有文件按 basename 记录大小：恢复逻辑据此跳过已被物化的同名同大小文件。 */
async function currentRootFileSizes(root: string): Promise<Map<string, number>> {
  const sizes = new Map<string, number>()
  const visit = async (directory: string, depth: number): Promise<void> => {
    if (depth > maxManagedArtifactDepth) {
      return
    }
    let entries
    try {
    ...
    for (const entry of entries) {
      ...
      const fileStat = await lstat(absolutePath).catch(() => null)
      if (fileStat?.isFile() && !fileStat.isSymbolicLink()) {
        sizes.set(entry.name, fileStat.size)
      }
    }
  }
  await visit(root, 0)
...
/**
 * bundle 条目去重：同名（剥掉 `-N` 数字后缀）+ 同大小 + 同 mime = 同内容双条目
 * （恢复逻辑与物化逻辑对同一文件重复归档，如 `wusong_tiger.png` 与 `wusong_tiger-2.png`）。
 * 同内容双条目保留无 `-N` 后缀的干净名（`-2` 是恢复/物化重复产物）；不删磁盘文件。
 */
function deduplicatedArtifactItems(items: readonly ArtifactItem[]): ArtifactItem[] {
  const seen = new Map<string, ArtifactItem>()
  for (const item of items) {
    const base = item.name.replace(/-\d+(?=\.[^.]+$)/u, "")
    const key = `${base}\0${item.size ?? ""}\0${item.mime}`
    const existing = seen.get(key)
    if (!existing) {
      seen.set(key, item)
      continue
    }
    if (item.name === base && existing.name !== base) {
      seen.set(key, item)
    }
  }
  return [...seen.values()]
...
export async function recoverMisplacedTurnArtifacts(
  ...
  // 当前轮已有文件（含 materializeAssistantArtifacts 刚物化的）按 basename 记大小：
  // 同名同大小 = 同一文件已被物化，恢复不再复制第二份（否则同图出现 -2 双条目）。
  const existingSizes = await currentRootFileSizes(resolvedArtifactRoot)
  for (const [relativePath] of changed.sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (existingSizes.get(path.basename(relativePath)) === sourceStat.size) {
      continue
    }
    ...
    try {
      ...
      console.warn(
        "[dweis] failed to recover misplaced turn artifact",
  ...
  const materializeNextArtifact = async (): Promise<void> => {
    while (nextArtifactIndex < artifacts.length) {
      ...
      try {
        ...
        console.warn(
          "[dweis] failed to materialize assistant artifact",
          error instanceof Error ? error.name : "unknown error",
        )
      }
    }
...
export function buildArtifactBundleFromGroup(input: {
  ...
  const items = deduplicatedArtifactItems(
  ...
  return {
    ...
    display,
    items,
    // 去重后条目数可能与磁盘计数不一致（双条目），未截断时以展示条目数为准。
    totalItems: group.truncated ? group.totalItems : items.length,
