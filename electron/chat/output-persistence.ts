...
export class OutputPersistence {
    ...
    await this.enqueueMutation(async (revision) => {
      ...
      try {
        ...
        if (rollbackErrors.length > 0) {
          ...
          console.warn("[dweis] failed to roll back output persistence session removal:", rollbackError)
          logDiagnostic(
            "chat-service",
  ...
  private async enqueueMutation(mutation: (revision: number) => Promise<void>): Promise<void> {
    ...
    const queued = this.mutationQueue
      .catch((error: unknown) => {
        console.warn("[dweis] previous output persistence mutation failed:", error)
