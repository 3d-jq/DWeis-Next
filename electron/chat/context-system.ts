...
export function buildProjectContextSystem(project: ChatProjectContext | undefined): string | undefined {
  ...
  const lines = [
    ...
    "- The shell and file tool cwd may still be DWeis's private scratch workspace; use this project directory as an absolute path instead of assuming cwd.",
    "- For Node.js project dependency commands, make this directory explicit with `cd <project-directory> && <package-manager> ...` or the package manager's explicit project-directory option. For Python, use the exact `<project-directory>/.venv` or `<project-directory>/venv` interpreter, directly or through `uv pip --python`; bare pip does not prove the target environment. Direct packages with no explicit source override can be approved automatically regardless of popularity, while other project dependency operations can receive a current-task approval. Do not use global installation, a custom registry, a user config, or a Git/URL/local package source unless the user explicitly asks.",
...
export function buildPermissionModeSystem(mode: AgentPermissionMode | undefined, browserAvailable: boolean): string {
  ...
  const lines = [
    ...
    `- Prefer the simplest reliable path across direct answers, local shell/files, ${branding.appName} Link tools, ${branding.appName}-controlled app APIs, concrete URL fetching, and selected local context.`,
    ...
    `- Concrete non-sensitive files, ordinary bounded directories, and shallow directory listings may also be approved automatically by ${branding.appName}, including paths outside the selected project when the task calls for them.`,
    ...
    `- Do not ask the user to approve ordinary local tool calls or switch modes. If ${branding.appName} pauses for a protected operation, ask only for that specific operation.`,
  ]
  if (browserAvailable) {
    lines.push(
...
export function buildResponseLanguageSystem(
  ...
  const detection = detectedLanguage
    ? `- DWeis has classified the latest user instruction as ${detectedLanguage}. Respond in ${detectedLanguage} unless the user explicitly requests a different response language. This classification takes priority over the application interface language. When delegating work through the task tool, explicitly require ${detectedLanguage} in the task prompt. Never present a subagent result in a different language; translate or rewrite it into ${detectedLanguage} before showing it to the user.`
    : "- DWeis could not classify the latest instruction with high confidence. Infer its language from the instruction itself and the rules below."
  return [
