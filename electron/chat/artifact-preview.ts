...
export function isPptxArtifact(filePath: string, mime: string): boolean {
  return (
    mime.toLowerCase() === "application/vnd.openxmlformats-officedocument.presentationml.presentation" ||
    extensionFromPath(filePath) === ".pptx"
  )
}
...
export function isBinaryDataPreviewArtifact(filePath: string, mime: string): boolean {
  return isPdfArtifact(filePath, mime) || isDocxArtifact(filePath, mime) || isPptxArtifact(filePath, mime)
}
...
export function binaryDataPreview(
  ...
  if (isPptxArtifact(filePath, mime)) {
    return { kind: "document", mime, size, documentFormat: "pptx", dataUrl: dataUrl(mime, bytes) }
  }
  return null
}
