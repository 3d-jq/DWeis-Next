...
import { DWEIS_MANAGED_PYTHON_ENV_DIRNAME } from "../agent/python-environment.ts"
  ...
  async function visit(dir: string, depth: number): Promise<void> {
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }))) {
      ...
      if (
        ...
        Date.now() >= deadline ||
        entry.name === ".DS_Store" ||
        entry.name === DWEIS_MANAGED_PYTHON_ENV_DIRNAME
      ) {
        continue
      }
...
export async function projectOutputFiles(
  ...
  const result = await collectGitTurnDiffs(baseline, mimeFromPath).catch((error: unknown) => {
    console.warn("[dweis] failed to collect project diff", error)
