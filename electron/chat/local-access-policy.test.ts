...
test("default access allows package runners unless they cross an explicit confirmation boundary", () => {
  ...
  for (const command of [
    ...
    'cd "/Users/test/Library/Application Support/dweis/agent/process/task" && npx md-to-pdf ' +
      '"/Users/test/Library/Application Support/dweis/agent/artifacts/report.md" ' +
      '--stylesheet "/Users/test/Library/Application Support/dweis/agent/process/task/pdf-style.css" ' +
      '--output "/Users/test/Library/Application Support/dweis/agent/artifacts/report.pdf" 2>&1',
  ]) {
    assert.deepEqual(
      evaluateLocalAccessRequest(permission({ metadata: { command } }), {
        permissionMode: "default",
...
test("local access policy allows requests in full access mode", () => {
  assert.deepEqual(
    evaluateLocalAccessRequest(permission({ metadata: { command: "rm -rf /tmp/dweis-test" } }), {
      permissionMode: "full_access",
    }),
