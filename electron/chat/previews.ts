...
function logPreviewFailure(scope: string, filePath: string, error: unknown, mime?: string): void {
  ...
  console.error(`[dweis] ${scope} failed`, { error: errorMessage(error), path: safePath })
...
export async function localArtifactPreview(
  ...
  convertPptxToPdf?: (pptxPath: string) => Promise<string | null>,
  ...
  if (isBinaryDataPreviewArtifact(item.path, item.mime) && size <= richPreviewMaxBytes) {
    ...
    if (
      ...
      if (resource && isPptxArtifact(item.path, item.mime)) {
        // 优先用 LibreOffice 转 PDF 完整渲染（图形/图片/排版）；soffice 不可用/失败回退文本大纲。
        if (convertPptxToPdf) {
          const pdfPath = await convertPptxToPdf(item.path)
          if (pdfPath && createResourceUrl) {
        ...
        return { kind: "document", mime: item.mime, size, documentFormat: "pptx", ...resourceResult(resource) }
      }
    }
    try {
      const bytes =
