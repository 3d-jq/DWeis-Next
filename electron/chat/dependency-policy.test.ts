...
test("package runner arguments are not mistaken for alternate package sources", () => {
  const fileArguments =
    ...
    '--output "/Users/test/Library/Application Support/dweis/agent/artifacts/report.pdf"'
  for (const runner of [
    "npx md-to-pdf",
    "npm exec md-to-pdf --",
    "npm x md-to-pdf --",
    ...
    const command = `cd "/Users/test/Library/Application Support/dweis/agent/process/task" && ${runner} ${fileArguments} 2>&1`
