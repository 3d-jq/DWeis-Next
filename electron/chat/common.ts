import type { DWeisAgentMode } from "../agent/mode.ts"
...
export type ReasoningLevel = DWeisReasoningLevel
export type AgentMode = DWeisAgentMode
...
export interface MessageStartedEvent {
  ...
  finishReason?: string
  completedAt?: number
  /** 该消息回合的 prompt token 实测（message.updated 的 info.tokens）；对齐 opencode footer 实时刷新。 */
...
export interface ChatMessage {
  ...
  clientId?: string
  /** 内部消息（如压缩 summary）：不渲染成对话内容，但保留 tokenUsage 等元数据供上下文占用计算。 */
  internal?: boolean
  /** DWeis 侧展示元数据：本轮发送时显式选择的 skill / connection 上下文。 */
...
export const ChatService = serviceName("chat-service") as ServiceName<{
  ...
  ClientInvokes: {
    ...
    redoSession(sessionId: string): Promise<void>
    ...
    /** 读取最新计划文件（.opencode/plans/ 下时间戳最新的 .md）供计划面板展示；无计划文件返回 null。 */
    getPlanMarkdown(): Promise<string | null>
