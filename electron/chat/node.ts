...
/** 压缩成功后「已完成上下文压缩」分隔线在对话流中的可见时长（ms）。 */
const compactionFinishedDisplayMs = 1_200
const defaultMaxDirectoryItems = 80
const startedMessageLimit = 5_000

/**
 * 斜杠模板命令的 system 指令：用户消息保持命令名（对话框显示 /命令），
 * 模板作为 system 段注入 agent（/init /review /自定义命令 .opencode/command/*.md）。
 * /init /review 支持参数（如 /review <commit>），模板里的 $ARGUMENTS 被替换为实际参数。
 */
async function slashCommandSystemPrompt(agent: AgentManager | null, text: string): Promise<string | undefined> {
  const trimmed = text.trim()
  if (trimmed === "/init" || trimmed.startsWith("/init ")) {
    const args = trimmed.slice("/init".length).trim()
    return args ? initCommandTemplate.replaceAll("$ARGUMENTS", args) : initCommandTemplate
  }
  if (trimmed === "/review" || trimmed.startsWith("/review ")) {
    const args = trimmed.slice("/review".length).trim()
    return args
      ? reviewCommandTemplate.replaceAll("$ARGUMENTS", args)
      : reviewCommandTemplate.replace("Input: $ARGUMENTS", "Input: （未指定，审查当前未提交变更）")
  }
  if (trimmed.startsWith("/") && agent) {
    let custom: CustomCommand[] = []
    try {
      custom = await agent.getCustomCommands()
    } catch {
      return undefined
    }
    const match = custom.find((command) => {
      const prefix = `/${command.name}`
      return trimmed === prefix || trimmed.startsWith(`${prefix} `)
    })
    if (match) {
      const args = trimmed.slice(`/${match.name}`.length).trim()
      return args ? match.template.replaceAll("$ARGUMENTS", args) : match.template
    }
  }
  return undefined
}
...
async function removeUnsubmittedTurnDirectories(
  ...
  await Promise.all(
    ...
    console.warn("[dweis] failed to clean unsubmitted turn directories", error)
  })
}
...
interface ChatServiceDeps {
  ...
  /** pptx → pdf 转换（LibreOffice headless）；返回转换后 pdf 路径或 null（不可用/失败）。 */
  createPptxPdfPreview?: (pptxPath: string) => Promise<string | null>
  ...
  /** 计划文件目录（workspace/.opencode/plans）；供计划面板读取最新计划。 */
  planDir?: string
  /** 一轮生成正常完成后通知主进程（后台记忆审查等）；停止和错误路径不触发。 */
  onTurnCompleted?: (input: { sessionId: string; messageId?: string }) => Promise<void> | void
}
...
export class ChatServiceImpl extends ConnectionService<ChatService> implements IConnectionService<ChatService> {
  ...
  public setAgentStatus(status: AgentRuntimeStatus): void {
    ...
    void this.send("agentStatusChanged", { status }).catch((error: unknown) => {
      console.warn("[dweis] failed to emit agent status:", error)
  ...
  /** 子代理会话 id → 展示用父会话 id（无映射原样返回）。供浏览器等按会话隔离的子系统归一化。 */
  public displaySessionIdFor(sessionId: string): string {
    return this.subagentSessions.displaySessionId(sessionId)
  }
      ...
      for (const translated of translateOpencodeEvent(event)) {
        ...
        if (translated.event === "agentError" && userStoppedSessionId) {
          ...
          if (messageId) {
            void this.rememberStoppedGeneration(sessionId, messageId, partIds, stoppedAt).catch((error: unknown) => {
              console.warn("[dweis] failed to record stopped generation", error)
            })
          }
          void this.finalizeTurnOutput(sessionId, messageId)
            .catch((error: unknown) => {
              console.warn("[dweis] failed to finalize stopped turn output", error)
            })
            .finally(() => {
              this.clearSessionGeneration(sessionId)
              this.activeAssistantMessages.delete(sessionId)
        ...
        if (translated.event === "messageStarted") {
          ...
          if (translated.data.role === "user" && this.compactingSessions.has(translated.data.sessionId)) {
            this.rememberInternalMessage(translated.data.sessionId, translated.data.messageId)
            // 压缩收尾时 session.idle 会清掉 compactingSessions；该触发消息后续的
            // message.updated（补 summary:{diffs} 等）会再次生成 messageStarted，
            // 若不去重就会把空白 user 气泡泄漏给渲染层。这里记录 started 标记，
            // 让同一消息的后续 messageStarted 走 rememberMessageStarted 去重。
            this.rememberMessageStarted(translated)
            continue
          }
        ...
        if (translated.event === "toolCallStarted") {
          ...
          if (childSessionId) {
            ...
            void this.agent
              ...
              .catch((error: unknown) => {
                console.warn("[dweis] failed to inherit task subagent knowledge scope:", error)
              })
          }
        }
        if (translated.event === "toolCallResult") {
          ...
          if (childSessionId) {
            ...
            void this.agent?.clearSessionKnowledgeBaseIds(childSessionId).catch((error: unknown) => {
              console.warn("[dweis] failed to clear task subagent knowledge scope:", error)
            })
          }
          if (translated.data.authorization) {
            void this.rememberAuthorizationOverlay(
              ...
              console.warn("[dweis] failed to record authorization overlay", error)
            })
          }
        }
        ...
        if (translated.event === "messageCompleted") {
          const sessionId = translated.data.sessionId
          // 压缩会话：internal 标记（compaction 触发消息 / summary 消息）保留到
          // compactSession finally 才清。session.idle 之后 sidecar 还会补发收尾事件
          // （触发消息最终 message.updated、summary 消息 text part 最终更新），
          // 若在这里清掉标记，这些事件会以 messageStarted/messageDelta 泄漏到渲染层
          // （压缩总结文本会作为 assistant 消息显示在对话里）。
  ...
  private sendBestEffort(
    ...
    void emit(event, data).catch((error: unknown) => {
      console.warn("[dweis] failed to emit chat server event:", { event, error, ...context })
      logDiagnostic(
        "chat-service",
  ...
  private answerLocalAccessPermission(
    ...
    void this.agent
      ...
      .catch((error: unknown) => {
        console.warn("[dweis] failed to approve local access permission:", error)
        logDiagnostic(
          "chat-service",
  ...
  private async autoAnswerPendingPermissions(
    ...
    try {
      ...
      console.warn("[dweis] failed to inspect pending permissions:", error)
  ...
  private async loadAdditionalTrustedRoots(): Promise<Iterable<string>> {
    ...
    try {
      ...
      console.warn("[dweis] failed to read trusted project roots:", error)
  ...
  private async completeSessionGeneration(
    ...
    try {
      ...
      await this.finalizeTurnOutput(sessionId, messageId).catch((error: unknown) => {
        console.warn("[dweis] failed to finalize turn output", error)
      })
      if (!this.isCurrentGeneration(sessionId, generation.id)) return
      ...
      if (completedRun) {
        void Promise.resolve(
          this.deps.onSessionCompleted?.({
            teamId: completedRun.workspace.kind === "team" ? completedRun.workspace.teamId : "",
            runId: completedRun.runId,
            sessionId,
          }),
        ).catch((error: unknown) => {
          console.warn("[dweis] failed to record completed task attention:", error)
        })
      }
    } finally {
  ...
  private clearSessionGeneration(sessionId: string, generationId?: string): void {
    ...
    if (agent) {
      void Promise.all([
        ...
        console.warn("[dweis] failed to clear session agent scope:", error)
      })
    }
  }
  ...
  private scheduleGenerationStartWatchdog(sessionId: string, generationId: string): void {
    this.generations.scheduleAcknowledgementWatchdog(sessionId, generationId, generationStartAckTimeoutMs, () => {
      console.warn("[dweis] generation did not receive an OpenCode event before timeout:", { sessionId })
  ...
  private scheduleGenerationSubmitWatchdog(sessionId: string, generationId: string): void {
    this.generations.scheduleAcknowledgementWatchdog(sessionId, generationId, generationSubmitTimeoutMs, () => {
      console.warn("[dweis] generation was not accepted by OpenCode before timeout:", { sessionId })
  ...
  private scheduleGenerationInactivityWatchdog(sessionId: string): void {
    ...
    this.generations.scheduleInactivityWatchdog(sessionId, timeoutMs, () => {
      ...
      console.warn("[dweis] generation stopped receiving OpenCode events before completion:", {
        noticeKind,
        sessionId,
        timeoutMs,
      })
  ...
  private async stopSessionGeneration(sessionId: string, options: StopSessionGenerationOptions): Promise<void> {
    ...
    if (options.abortAgent) {
      try {
        ...
        console.warn("[dweis] generation abort failed:", error)
      }
    }
    ...
    if (options.reason === "user") {
      await this.send("generationStopped", {
        ...
        console.warn("[dweis] failed to emit generation stopped:", error)
  ...
  public async sendMessage(req: SendMessageRequest): Promise<void> {
    ...
    try {
      ...
      const artifactBaseline = await captureArtifactSessionBaseline(
        ...
        console.warn("[dweis] failed to capture artifact session baseline", error)
        logDiagnostic(
          "chat-service",
      ...
      // @文件引用：sidecar 的 @ 解析基于 agent workspace，把项目内 @ 相对路径解析为绝对路径
      // （仅转换真实存在的路径），用户才能在消息里 @ 引用项目文件（对齐 opencode 的 @ 引用）。
      const promptText = trustedProjectRoot ? await resolveAtMentionPaths(req.text, trustedProjectRoot) : req.text
      void this.agent
        .promptStreaming(req.sessionId, promptText, {
          ...
          system: mergeSystemPrompts(
            ...
            // 斜杠模板命令（/init /review /自定义命令）：用户消息保持命令名（对话框显示 /命令），
            // 模板作为 system 指令注入 agent，避免长指令裸露在对话流。
            await slashCommandSystemPrompt(this.agent, req.text),
            // bugReportSystem 含每轮 /bug-report 的 text，是动态段，放尾部。
            bugReportSystem,
          ),
          // 稳定段前置（权限规则）：随轮次不变，最大化 system 前缀缓存命中。
          stableSystem: mergeSystemPrompts(
  ...
  private async rollbackUnsubmittedUserAttachments(
    ...
    try {
      ...
      console.warn("[dweis] failed to roll back unsubmitted user attachments:", error)
      logDiagnostic(
        "chat-service",
  ...
  private async publishArtifactBundle(bundle: ArtifactBundle): Promise<void> {
    ...
    await this.send("artifactBundleUpdated", { sessionId: bundle.sessionId, messageId: bundle.messageId }).catch(
      (error: unknown) => {
        console.warn("[dweis] failed to emit artifact bundle update", error)
        logDiagnostic(
          "chat-service",
  ...
  private async projectBaseline(project: ChatProjectContext | undefined): Promise<{
    ...
    try {
      ...
      console.warn("[dweis] failed to capture project baseline", error)
      return {}
    }
  }
    ...
    await finalizeTurnOutputArtifacts({
      ...
      publishTurnOutput: async (record) => {
        ...
        await this.send("turnOutputUpdated", { sessionId, messageId: resolvedMessageId }).catch((error: unknown) => {
          console.warn("[dweis] failed to emit turn output update:", error)
          logDiagnostic(
            "chat-service",
  ...
  public async getLocalArtifactPreview(req: LocalArtifactPreviewRequest): Promise<LocalArtifactPreviewResult> {
    ...
    return localArtifactPreview(
      ...
      this.deps.createPptxPdfPreview,
    )
  }
  ...
  public async compactSession(sessionId: string): Promise<void> {
    ...
    // 压缩开始时先标记：sidecar 创建 compaction 用户消息的 messageStarted 会被
    // 事件桥按 compactingSessions 过滤（node.ts L514），否则渲染层会出现空白 user 气泡。
    this.compactingSessions.add(sessionId)
    try {
      await this.agent.summarizeSession(sessionId)
      // 压缩成功：session.compacted → resuming 活动让分隔线显示「已完成上下文压缩」。
      // 延迟 messageCompleted，让完成态在对话流中短暂可见（对齐 opencode 的 summary 元信息行）。
      await new Promise((resolve) => setTimeout(resolve, compactionFinishedDisplayMs))
    } finally {
      this.compactingSessions.delete(sessionId)
      this.clearInternalMessages(sessionId)
      // 压缩不走 generation 流程：sidecar 的 session.idle → messageCompleted 在事件桥
      // 被 consume 且因无 generation 不会转发（L626-632）。这里本地补发，保证渲染层
      // status 回到 ready、activity 清空、触发 reload，UI 不会卡在 streaming。
      this.sendBestEffort(this.send.bind(this), "messageCompleted", { sessionId }, { sessionId })
    }
  }

  /** 撤销到最后一条用户消息（该消息及之后被回滚；对齐 opencode /undo）。 */
  public async undoSession(sessionId: string): Promise<void> {
    if (!this.agent) {
      return
    }
    const messages = await this.agent.getMessages(sessionId)
    const lastUser = [...messages].reverse().find((message) => message.role === "user" && !message.internal)
    if (!lastUser) {
      return
    }
    await this.agent.revertToMessage(sessionId, lastUser.id)
  }

  /** 恢复被撤销的变更（对齐 opencode /redo）。 */
  public async redoSession(sessionId: string): Promise<void> {
    if (!this.agent) {
      return
    }
    await this.agent.unrevertSession(sessionId)
  }

  /** 扫描 agent workspace 的自定义斜杠命令（.opencode/command/*.md）。 */
  public async getCustomCommands(): Promise<CustomCommand[]> {
    return this.agent ? this.agent.getCustomCommands() : []
  }

  /** 用户直接执行 shell 命令并写入会话（对齐 opencode shell 直执行）。 */
  public async runShellCommand(sessionId: string, command: string): Promise<void> {
    if (!this.agent) {
      return
    }
    await this.agent.runShellCommand(sessionId, command)
  }

  public async getPlanMarkdown(): Promise<string | null> {
    if (!this.deps.planDir) {
      return null
    }
    try {
      const entries = await readdir(this.deps.planDir, { withFileTypes: true })
      ...
      return await readFile(path.join(this.deps.planDir, planFiles[0].name), "utf-8")
    } catch (error) {
      if (!isMissingFileError(error)) {
        console.warn("[dweis] failed to read plan file:", error)
      }
      return null
    }
  }
    ...
    if (req.reply === "always") {
      if (!request) {
        try {
          ...
          console.warn("[dweis] failed to inspect permission before saving session grant:", error)
          logDiagnostic(
            "chat-service",
