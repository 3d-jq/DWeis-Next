...
/**
 * 剥离 here-document payload（`<<DELIM ... DELIM` 块内容），保留前后的 shell 命令。
 * here-doc 内容是数据（Python 代码 / HTML / 文本），不是 shell 语法——把其中的 `/`、
 * 路径当文件系统访问会误判（对齐 opencode #311：`monthly_new / (1 - retention)` 的
 * 除法 `/` 被当成访问根目录，普通分析/生成被反复要求确认）。
 */
export function stripShellHereDocs(command: string): string {
  ...
  for (const line of lines) {
    ...
    // 识别行内 `<<EOF` / `<<'EOF'` / `<<"EOF"` / `<<-EOF` / `<<\EOF`，进入 here-doc 模式。
    const match = /(?:^|[\s&|;()])<<-?(?:(['"])(.*?)\1|\?([A-Za-z_][A-Za-z0-9_]*))/.exec(line)
    if (match) {
      delimiter = match[2] ?? match[3] ?? null
      kept.push(line)
    } else {
      kept.push(line)
    }
