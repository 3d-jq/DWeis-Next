...
test("localArtifactPreview only grants leases for resource-backed previews", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "dweis-preview-leases-"))
  try {
    ...
    const createResourceUrl = () => {
      grants += 1
      return { expiresAt: 123, url: "dweis-resource://artifact/token" }
    }
...
test("localArtifactPreview rejects an unsafe DOCX before granting a resource lease", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "dweis-preview-docx-safety-"))
  try {
...
test("localArtifactPreview routes CSV and TSV through the spreadsheet preview worker", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "dweis-delimited-preview-"))
  try {
