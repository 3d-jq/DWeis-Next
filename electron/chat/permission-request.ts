...
function commandAccessResources(command: string, depth = 0): string[] {
  // 剥离 here-doc payload 再提取资源：here-doc 内容是数据，其中的 `/`/路径不是 shell 文件访问
  // （对齐 opencode #311，避免 Python 除法等被误判为访问根目录）。
  return topLevelShellSegments(stripShellHereDocs(command)).flatMap(({ text }) => {
...
export function isHighRiskPermissionRequest(request: ChatPermissionRequest): boolean {
  ...
  const command = stripShellHereDocs(commandText(request) ?? "")
  if (!command) {
    return false
  }
  return (
...
/**
 * Recognizes direct PyPI requirements installed through DWeis's private per-task environment.
