...
async function openPlainArtifactFile(artifactRoot: string, source: string): Promise<FileHandle> {
  ...
  try {
    ...
    // Windows 上 lstat 的 dev 恒为 0，且 ino 在 lstat/fstat 间不可靠（源文件被替换后
    // 两者仍相同，dev/ino 指纹静默失效）；size + mtimeMs 是跨平台可靠指纹——
    // 替换/修改源文件必然改变 size 或 mtime。非 win32 保留 dev/ino 作为额外指纹。
    if (
      ...
      pathInfo.mtimeMs !== openedInfo.mtimeMs ||
      (process.platform !== "win32" && (pathInfo.dev !== openedInfo.dev || pathInfo.ino !== openedInfo.ino))
    ) {
