...
test("spreadsheet preview worker parses an XLSX end to end", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "dweis-xlsx-worker-"))
...
test("spreadsheet preview worker parses CSV and TSV end to end", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "dweis-delimited-worker-"))
  const worker = new Worker(new URL("./spreadsheet-preview-worker.ts", import.meta.url), {
    execArgv: ["--experimental-strip-types"],
  })
  try {
    const fixtures = [
      { content: 'name,note\nDWeis,"hello, world"', extension: "csv", mime: "text/csv" },
      { content: "name\tnote\nDWeis\thello", extension: "tsv", mime: "text/tab-separated-values" },
    ] as const
