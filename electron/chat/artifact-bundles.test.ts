...
test("buildArtifactBundle infers an image gallery without a model-authored manifest", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-bundle-"))
  try {
...
test("buildArtifactBundle omits resumable task state beside final image outputs", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-session-state-"))
  try {
...
test("buildArtifactBundle keeps legitimate and uncertain JSON deliverables", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-json-deliverables-"))
  try {
...
test("buildArtifactBundle keeps a lone operational state file as the only output", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-only-state-"))
  try {
    await writeFile(
...
test("buildArtifactBundle keeps explicitly materialized state-shaped attachments", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-explicit-state-"))
  try {
...
test("buildArtifactBundle records a visible generated image that was not persisted", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-bundle-failed-"))
  try {
    const bundle = await buildArtifactBundle({
      artifactRoot: root,
      completedAt: 2,
          ...
          parts: [
            {
              ...
              partId: "text-1",
              text: [
                "![result](/Users/me/Application Support/dweis/result.png)",
                "```md",
...
test("buildArtifactBundle marks incomplete preview attribution as unverified", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-bundle-partial-"))
  try {
    await writeFile(path.join(root, "001.png"), "one")
...
test("buildArtifactBundle does not let an unrelated image mask a failed generated preview", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-unrelated-image-"))
  try {
...
test("materializeAssistantArtifacts copies assistant files into managed storage", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-materialize-"))
  try {
...
test("materializeAssistantArtifacts records an image source already inside managed storage", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-already-managed-"))
  try {
...
test("materializeAssistantArtifacts prefers a duplicate markdown image over a non-image attachment", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-duplicate-source-"))
...
test("materializeAssistantArtifacts turns a data image preview into a ready artifact", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-data-preview-"))
  try {
    const origins = await materializeAssistantArtifacts(
      [
        {
...
test("materializeAssistantArtifacts downloads an HTTPS image preview into the artifact bundle", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-https-preview-"))
  try {
    let resolutions = 0
...
test("materializeAssistantArtifacts refuses HTTPS image previews that resolve to private addresses", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-private-preview-"))
  try {
    let requested = false
    const origins = await materializeAssistantArtifacts(
      [
...
test("materializeAssistantArtifacts rejects reserved and documentation address ranges", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-reserved-preview-"))
  try {
    let requests = 0
...
test("materializeAssistantArtifacts bounds source count and download concurrency", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-concurrency-preview-"))
  try {
...
test("materializeAssistantArtifacts refuses redirects from public image URLs into private addresses", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-private-redirect-"))
  try {
    let requests = 0
    const origins = await materializeAssistantArtifacts(
      [
...
test("recoverMisplacedTurnArtifacts copies only files created or changed in old turn directories", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-recovery-"))
  try {
...
test("recoverMisplacedTurnArtifacts skips files already materialized with the same name and size", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-recovery-dedup-"))
  try {
    const sessionRoot = path.join(root, "session-1")
    const oldTurn = path.join(sessionRoot, "old-turn")
    const currentTurn = path.join(sessionRoot, "current-turn")
    ...
    await mkdir(currentTurn)
    // 当前轮已有一张同名同大小图片（materializeAssistantArtifacts 刚物化）
    await writeFile(path.join(currentTurn, "wusong_tiger.png"), "same-bytes")

    const baseline = await captureArtifactSessionBaseline(sessionRoot, currentTurn)
    assert.ok(baseline)
    // 模型把同一张图又写进旧轮目录：恢复逻辑不应再复制出 -2 双份
    await writeFile(path.join(oldTurn, "wusong_tiger.png"), "same-bytes")
    ...
    assert.equal(bundle?.items[0]?.name, "wusong_tiger.png")
    // 磁盘上不应出现 -2 副本
    assert.equal(await readFile(path.join(currentTurn, "wusong_tiger.png"), "utf8"), "same-bytes")
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})

test("buildArtifactBundle deduplicates -N twin entries of the same content", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-bundle-dedup-"))
  try {
    // 模拟历史双条目：同内容同大小，仅 -N 后缀不同
    await writeFile(path.join(root, "wusong_tiger.png"), "same-bytes")
    await writeFile(path.join(root, "wusong_tiger-2.png"), "same-bytes")
...
test("ArtifactBundleStore read deduplicates persisted twin entries", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-store-dedup-"))
  try {
    ...
    const baseItem = {
      id: "i1",
      path: path.join(root, "wusong_tiger.png"),
      name: "wusong_tiger.png",
      kind: "file" as const,
      mime: "image/png",
      size: 9,
    ...
    const records = await new ArtifactBundleStore(root).read()
    const bundle = records.get(sessionId)?.get(messageId)
    assert.equal(bundle?.items.length, 1)
    assert.equal(bundle?.items[0]?.name, "wusong_tiger.png")
    assert.equal(bundle?.totalItems, 1)
  } finally {
...
test("artifact session recovery ignores symlinks and roots outside the captured session", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-recovery-boundary-"))
  try {
    const sessionRoot = path.join(root, "session-1")
    const oldTurn = path.join(sessionRoot, "old-turn")
    const currentTurn = path.join(sessionRoot, "current-turn")
...
test("ArtifactBundleStore round trips records and removes a session", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "dweis-artifact-bundle-store-"))
  try {
