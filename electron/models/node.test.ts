...
test("ModelsServiceImpl preserves custom model image support on update", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "dweis-models-service-"))
...
test("ModelsServiceImpl defaults known provider image support but honors user choices", async () => {
  ...
  assert.equal(qwen.customModels[0]?.inputTokenLimit, undefined)
  assert.equal(qwen.customModels[0]?.maxOutputTokens, undefined)
  ...
  assert.equal(geminiModel?.inputTokenLimit, undefined)
  assert.equal(geminiModel?.maxOutputTokens, undefined)
