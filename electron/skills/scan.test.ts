...
test("scanDWeisInstalledSkills reads shared Agent Skills and prefers app cache as source", async () => {
  const rootPath = await mkdtemp(path.join(os.tmpdir(), "dweis-shared-skills-"))
  ...
  try {
    ...
    const skills = await scanDWeisInstalledSkills({ cacheSkillStoreRoot, sharedSkillRoot })
    const managed = skills.find((skill) => skill.name === "managed")
    const uncached = skills.find((skill) => skill.name === "uncached")

    assert.equal(managed?.agent.id, "dweis")
