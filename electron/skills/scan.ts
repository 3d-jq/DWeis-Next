...
export const dweisRuntimeAgent: SupportedAgent = {
  cliCommands: [],
  homeRoot: "",
  id: "dweis",
  name: "DWeis",
...
export async function scanDWeisInstalledSkills(request: {
  cacheSkillStoreRoot: string
  sharedSkillRoot: string
}): Promise<InstalledSkill[]> {
  return scanInstalledSkillRoot({
    agent: dweisRuntimeAgent,
    resolveSourcePath: (skill) => resolveDWeisSkillSourcePath(skill, request.cacheSkillStoreRoot),
...
async function resolveDWeisSkillSourcePath(
  skill: { metadata: ManagedSkillMetadata; name: string; path: string },
  cacheSkillStoreRoot: string,
): Promise<string> {
  const sourceCandidates = readDWeisSkillSourceCandidates(skill, cacheSkillStoreRoot)
...
function readDWeisSkillSourceCandidates(
