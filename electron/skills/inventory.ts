...
const dweisRuntimeHostId = "dweis"

function isDWeisRuntimeAgent(agent: SupportedAgent): boolean {
  return agent.id === dweisRuntimeHostId
}
  ...
  return agents.map((agent) => {
    ...
    if (!installedSkill) {
      return {
        ...
        scope: isDWeisRuntimeAgent(agent) ? "runtime" : "external",
        status: "missing",
      }
    }
