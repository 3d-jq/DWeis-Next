...
/** 把外部 agent skills 镜像到 DWeis 私有 runtime，并串行化扫描、复制、manifest 与陈旧镜像清理。 */
export class ExternalSkillRuntimeSynchronizer {
  ...
  private async syncNow(removedStore: RemovedStore): Promise<boolean> {
    ...
    for (const skill of sortedSkills) {
      ...
      try {
        ...
        console.warn("[dweis] failed to sync external skill to runtime:", {
    ...
    for (const result of reconciliation.skipped) {
      console.warn("[dweis] skipped stale external runtime skill cleanup:", result)
      logDiagnostic(
        "skills",
  ...
  private async writeManifestRecord(skill: InstalledSkill, targetPath: string): Promise<void> {
    ...
    const records = manifestStore.records.filter(
      (record) => !(record.agentId === "dweis" && record.installedPath === targetPath),
    )
    records.push({
      agentId: "dweis",
      hash: skill.hash,
...
function readRuntimeMirrorManifestRecord(
  ...
  return manifestStore.records.find((record) => record.agentId === "dweis" && record.installedPath === targetPath)
}
